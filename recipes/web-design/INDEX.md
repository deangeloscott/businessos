# Web Design & Conversion Architecture

A compact menu for four reusable web tracks. Open the track that matches the commercial job, then use its sections as adaptable starting points. The optional notes preserve shared marketing, persuasion, technical, and diagnostic knowledge without making every page brief carry the entire library.

## Choose a track

| Track | Use when | Open |
|---|---|---|
| E-commerce store | Building catalog, product detail, cart, or checkout experiences. | [01 — E-commerce stores](01-ecommerce-stores.md) |
| Lead generation | Creating lead magnets, forms, quizzes, applications, or booking flows. | [02 — Lead generation](02-lead-generation.md) |
| Brand and business site | Clarifying positioning, navigation, personas, proof, or authority. | [03 — Brand and business websites](03-brand-business-websites.md) |
| Multi-step sales funnel | Structuring advertorials, book funnels, challenges, webinars, VSLs, community, or upsells. | [04 — Multi-step sales funnels](04-multistep-sales-funnels.md) |

## Optional shared notes

Open only the note that materially improves the current starting point:

- [Marketing principles and section patterns](notes/marketing-principles.md) — frameworks, reusable web sections, awareness, and a seven-block brief.
- [Persuasion, offers, and risk framing](notes/persuasion-and-offers.md) — CTA language, reassurance, urgency, scarcity, guarantees, and comparison modes.
- [Technical and machine-readable patterns](notes/technical-and-machine.md) — performance, semantic HTML, structured data, and conditional machine discoverability.
- [Diagnostics, market awareness, and customer language](notes/diagnostics-and-copy.md) — hypotheses, sophistication, and voice-of-customer inputs.
- [Dialects](DIALECTS.md) — ten optional visual systems and design tokens.
- [Directory, workflow links, and references](notes/references.md) — navigation, source map, and targeted AURA workflow links.

## Fast use

1. Name the audience, traffic source, commercial job, offer, evidence, platform, and desired action.
2. Open one track and borrow only the sections that answer the visitor's real decision questions.
3. Follow an optional note or AURA workflow when it supplies missing judgment, evidence, or QA; adapt or skip it when it does not.
4. Inspect the implemented page on the actual devices, states, and integrations that matter.

The web recipes are optional craft references. Existing AURA workflows own organizational truth, evidence, accessibility, and workflow-level QA; this library supplies reusable web concepts and starting briefs.
