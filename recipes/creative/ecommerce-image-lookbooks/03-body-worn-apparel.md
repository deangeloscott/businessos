# Scale 03: Body-Worn & Apparel E-Commerce Lookbook
### Master Archetypes for Products Worn on the Human Body
*Categories: Fashion, Footwear, Eyewear, Bags & Luggage, Outerwear, Activewear, Streetwear*

---

## Overview & Scale Dynamics
Apparel, footwear, and accessories are high-volume, high-return categories where **fit mismatches, fabric hand-feel expectations, unrealistic digital rendering, and inaccurate color reproduction** can create costly friction. Use the organization's actual return and sizing data when quantifying the problem.

Every archetype in this scale is a reference for showing **fabric drape, color, movement, fit, and interior capacity**. Use the optional five-block scaffold when useful. Generic “cheap versus premium” contrasts may intensify the pain-to-relief story, but a rendered garment does not establish waterproofing, stretch recovery, durability, or fit for every body; use supplied tech packs, size data, samples, or documented tests.

---

### Archetype 01: The Ghost Mannequin & Architectural Studio Drape
* **Universal Class:** The Specimen
* **Example Products (Illustrative Guidance, Not an Allowlist):** Heavyweight cotton t-shirt, 3-layer technical shell, merino wool sweater, tailored wool blazer, denim jacket.
1. **Strategic Intent & Conversion Role:**  
   Shopify & Amazon Catalog Main (Slot 1). Provides complete, distraction-free view of garment silhouette, collar structure, sleeve drape, and hemlines without human model bias.
2. **The Physical Stage (The 95%):**  
   Pure seamless light-grey or neutral white cyclorama (RGB 248, 248, 248). The garment is styled on an invisible 3D ghost mannequin, showing interior neck label, lining, and authentic 3D hollow depth.
3. **Optical & Lighting Directives:**  
   * *Lens:* 85mm Prime.  
   * *Aperture:* f/8 for uniform front-to-back fabric sharpness.  
   * *Lighting:* Soft wrap-around studio lighting (large overhead diffused bank + two side bounce panels) showing true fabric drape and seam construction without harsh hot spots.
4. **Sensory & Synesthetic Triggers:**  
   Visual weight and structural stiffness/softness of the textile (e.g. heavy 400 GSM French terry vs lightweight breezy linen).
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Zero warping of garment proportions. Calibrated 5200K daylight balance ensuring fabric dye exactly matches physical Pantone under real sunlight.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO]: Commercial ghost mannequin packshot of [HERO PRODUCT: Garment type, color, textile material, GSM weight, collar/cuff construction]. Centered in frame, occupying 82% of frame height, showcasing natural 3D anatomical volume with hollow interior collar and neck label visible.
[BLOCK 2: OPTICAL RIG]: 85mm prime portrait lens, eye-level perspective, f/8 aperture for front-to-back edge sharpness from collar to hem.
[BLOCK 3: LIGHTING RIG]: Clean 5200K wrap-around studio softbox lighting with soft ambient fill, highlighting authentic fabric drape and micro-seam stitching with a soft contact floor shadow.
[BLOCK 4: SUBORDINATE STAGE]: Pure seamless neutral light-gray cyclorama background (calibrated RGB 248, 248, 248). Zero props, zero distractions.
[BLOCK 5: EXCLUSIONS]: Negative: human face, visible mannequin neck, plastic shine, distorted sleeves, uneven hemlines, wrinkled background.
```

---

### Archetype 02: The Streetwear Candid Movement
* **Universal Class:** The Ritual
* **Example Products (Illustrative Guidance, Not an Allowlist):** Oversized boxy hoodie, relaxed cargo pants, technical commuter coat, chunky leather sneakers, nylon cross-body bag.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 3 & Lifestyle Lookbook. Shows how the garment drapes, folds, and moves dynamically during natural walking motion in a real-world urban environment.
2. **The Physical Stage (The 95%):**  
   Urban architectural streetscape (brutalist concrete steps, clean city sidewalk, textured limestone facade). Model is captured mid-stride, turning, or casually leaning.
3. **Optical & Lighting Directives:**  
   * *Lens:* 50mm Prime at waist-level.  
   * *Aperture:* f/2.8 with subtle cinematic lens compression.  
   * *Lighting:* Natural open-shade daylight with soft directional sky reflection.
4. **Sensory & Synesthetic Triggers:**  
   Natural kinetic movement: the organic ripple of pants over sneakers, the swing of a jacket hem, effortless wearable comfort.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Model posture must be natural and unforced. Garment colors must not be contaminated by reflective colored buildings.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO]: Commercial streetwear editorial photograph of [HERO PRODUCT: Garment type, fabric, fit]. The garment is the primary focus, occupying 75% of the frame on a stylish model captured in a natural, candid walking stride.
[BLOCK 2: OPTICAL RIG]: 50mm prime lens at waist-level, f/2.8 aperture for shallow depth of field.
[BLOCK 3: LIGHTING RIG]: Natural open-shade 5400K daylight with soft directional skylight revealing true fabric drape, texture, and movement folds.
[BLOCK 4: SUBORDINATE STAGE]: Clean brutalist concrete architecture or urban sidewalk in the background, thrown 2 stops darker into creamy f/2.8 bokeh.
[BLOCK 5: EXCLUSIONS]: Negative: stiff runway poses, distorted limbs, blurry garment details, distracting graffiti, oversaturated background colors.
```

---

### Archetype 03: The Macro Fabric Weave & Seam Stitching
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** Heavyweight raw selvedge denim, merino wool knit, Cordura ballistic nylon backpack, waterproof taped zipper jacket, cashmere scarf.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 5 (Material Quality Explanation). Directly addresses fear of cheap, thin, synthetic fabric by making the supplied construction details visible. A macro view can illustrate the desired quality, while thread count and material claims still require the tech pack.
2. **The Physical Stage (The 95%):**  
   Extreme macro framing on a critical construction zone: the heavy ribbed collar, reinforced bar-tack stitching on a pocket, selvage denim edge, or waterproof taped zipper seam.
3. **Optical & Lighting Directives:**  
   * *Lens:* 105mm Macro.  
   * *Aperture:* f/5.6.  
   * *Lighting:* Raking low-angle cross light (30° from the side) to cast tiny shadows across each individual yarn, thread twist, and weave indentation.
4. **Sensory & Synesthetic Triggers:**  
   Tactile haptic simulation: the dry grain of raw denim, brushed softness of cashmere, or dense technical weave of ballistic Cordura nylon.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Thread count, stitch density, and seam thickness should follow the technical garment tech pack when stated; otherwise show texture without inventing measurements.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO MACRO]: Extreme macro textile photograph of [HERO PRODUCT TEXTILE: Weave type, fiber material, GSM weight, construction zone, e.g. 14oz raw selvedge denim or 8mm taped waterproof zipper]. Occupying 85% of frame in microscopic tack-sharp focus.
[BLOCK 2: OPTICAL RIG]: 105mm True Macro lens, f/5.6 aperture, locking focus onto individual thread twists, yarn cross-sections, and lockstitches.
[BLOCK 3: LIGHTING RIG]: Raking directional side-light at a 30-degree angle casting tiny micro-shadows that sculpt the physical texture and tactile depth of the weave.
[BLOCK 4: SUBORDINATE STAGE]: Seamless dark industrial surface, background falling off into pure optical blur.
[BLOCK 5: EXCLUSIONS]: Negative: flat blurry texture, digital noise, synthetic polyester shine, loose dangling frayed threads, plastic artifacts.
```

---

### Archetype 04: The Pocket & Interior Capacity Test
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** Commuter backpack, technical shell jacket, weekender duffel bag, EDC tactical pants, leather tote bag.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 4 (Utility De-risking for Bags, Jackets & Pants). Answers the critical functional question: *"Will this actually hold my phone, laptop, water bottle, or passport?"*
2. **The Physical Stage (The 95%):**  
   The bag, backpack, or outerwear piece with its main compartment or pocket held open. Authentic everyday gear (15" laptop, phone, passport, water bottle) is shown partially sliding in or neatly organized inside.
3. **Optical & Lighting Directives:**  
   * *Lens:* 35mm Prime looking down into the open pocket.  
   * *Aperture:* f/5.6.  
   * *Lighting:* Clean interior fill lighting illuminating deep into dark pocket linings so internal dividers are clearly visible.
4. **Sensory & Synesthetic Triggers:**  
   The immense psychological relief of organized utility: smooth-gliding metal zippers, padded fleece laptop sleeves, water-resistant interior coatings.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Real-world items used for scale must match universal dimensions (e.g. an actual 15-inch laptop, not an oversized or undersized prop).
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO & CAPACITY]: Commercial utility demonstration photograph of [HERO PRODUCT: Backpack, tote, or jacket interior], with main pocket unzipped to reveal interior storage capacity, occupying 80% of frame weight.
[BLOCK 2: REAL-WORLD SCALE]: A recognized daily object [SCALE OBJECT: e.g., sleek aluminum 15-inch laptop or water bottle] is shown smoothly sliding into the dedicated padded fleece compartment, proving ample dimensional clearance.
[BLOCK 3: OPTICS]: 40mm prime lens looking into the open compartment, f/5.6 aperture keeping interior dividers and zippers in sharp focus.
[BLOCK 4: STAGE & LIGHT]: Bright 5500K studio daylight with targeted interior fill light illuminating the dark lining fabric; grounded on a clean concrete or wood workbench.
[BLOCK 5: EXCLUSIONS]: Negative: dark unreadable pocket interior, cluttered messy junk, warped laptop dimensions, distorted zippers, messy background.
```

---

### Archetype 05: High-Fashion Direct Flash
* **Universal Class:** The Habitat
* **Example Products (Illustrative Guidance, Not an Allowlist):** Leather trench coat, luxury sunglasses, satin slip dress, designer sneakers, architectural sunglasses.
1. **Strategic Intent & Conversion Role:**  
   Paid Social (TikTok / Meta Feed pattern-interrupt) & Brand Campaign. Channels contemporary editorial energy (Balenciaga, Aimé Leon Dore, SKIMS).
2. **The Physical Stage (The 95%):**  
   Night-time urban street, concrete parking garage, or minimalist industrial interior. Model captured in a candid, high-status pose.
3. **Optical & Lighting Directives:**  
   * *Lens:* 35mm Prime.  
   * *Aperture:* f/8.  
   * *Lighting:* Hard, point-source direct flash mounted on-camera. Deep drop shadow immediately behind the model, zero ambient light wash.
4. **Sensory & Synesthetic Triggers:**  
   Auditory and cultural memory of fashion week, paparazzi energy, raw self-assured style.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Flash must not blow out fabric highlights (e.g. white shirts must retain textile texture and not become solid blinding white).
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO]: High-fashion editorial flash photograph of [HERO PRODUCT: Garment, color, fabric, silhouette] on a model posing with detached editorial confidence, occupying 75% of vertical frame height.
[BLOCK 2: OPTICAL RIG]: 35mm prime lens, eye-level angle, f/8 aperture with subtle 35mm analog film grain.
[BLOCK 3: LIGHTING RIG]: Hard, point-source direct on-camera paparazzi flash casting a sharp, graphic black drop shadow directly onto the back wall, 0% ambient fill.
[BLOCK 4: SUBORDINATE STAGE]: Minimalist raw concrete garage or moody urban night interior, background underexposed (-2 EV) to lock full contrast on the garment.
[BLOCK 5: EXCLUSIONS]: Negative: blown-out white fabric, cheesy commercial smile, distorted limbs, blurry details, soft pastel lighting.
```

---

### Archetype 06: The Gorpcore Weather & Element Torture Test
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** 3-layer GORE-TEX jacket, DWR trail hiking pants, all-weather trail runners, waterproof duffel.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 2 (Technical Performance Demonstration for Outerwear & Trail Gear). Makes water-repellency, windproofing, or ruggedness visually legible. Use a documented test for factual performance language; a generic ideal rain scene can dramatize the desired relief.
2. **The Physical Stage (The 95%):**  
   Damp outdoor mountain or trail setting: mist-covered granite boulders, rainfall, or cold alpine ridge. The jacket or shoe is shown actively repelling water.
3. **Optical & Lighting Directives:**  
   * *Lens:* 85mm Prime.  
   * *Aperture:* f/4.  
   * *Lighting:* Overcast mountain light with cold directional rim-lighting catching water droplets standing up (beading) on the fabric membrane.
4. **Sensory & Synesthetic Triggers:**  
   Tactile protection: hearing rain drumming on a tight hood, knowing you are dry and insulated inside; surface tension of perfectly spherical water beads rolling off DWR coating.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Water-beading, saturation, and seam behavior should match supplied fabric documentation or observed testing. If unknown, keep the scene illustrative and avoid naming a rating.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO & WEATHER PROOF]: Rugged outdoor performance photograph of [HERO PRODUCT: Technical shell jacket, trail shoes, or pack] in an intense mountain rainstorm. Product occupies 75% of the frame in tack-sharp focus.
[BLOCK 2: PERFORMANCE DETAIL]: Raindrops visibly bead or spread across the textured membrane according to the supplied fabric reference or documented test. A high-contrast ideal can be used for concept work, but do not imply waterproof performance from the generated frame alone.
[BLOCK 3: OPTICAL RIG]: 85mm prime lens, f/4 aperture, sharp focus on fabric texture and water beads.
[BLOCK 4: STAGE & LIGHT]: Cold 6000K overcast mountain daylight with soft directional rim lighting. Background is misty pine trees and wet granite, 2 stops darker.
[BLOCK 5: EXCLUSIONS]: Negative: saturated soaked wet fabric, cartoon water droplets, sunny sky, plastic raincoat look, unnatural model posture.
```

---

### Archetype 07: Multi-Body Fit Diversity (Sizing Reassurance)
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** Unisex fleece hoodie, tailored chinos, everyday button-down shirt, activewear leggings, denim jacket.
1. **Strategic Intent & Conversion Role:**  
   PDP Sizing Guide & Slot 4. Dramatically reduces return rates by showing how the same garment fits across different body types and heights.
2. **The Physical Stage (The 95%):**  
   Clean, neutral architectural studio with soft daylight. Two or three diverse models standing side-by-side wearing the garment with natural posture.
3. **Optical & Lighting Directives:**  
   * *Lens:* 50mm Prime at chest height to eliminate perspective distortion.  
   * *Aperture:* f/8 (all models in equal sharpness).  
   * *Lighting:* Broad, flat, flattering studio light.
4. **Sensory & Synesthetic Triggers:**  
   Instant customer confidence: *"She looks like my body type—I know exactly which size to order now."*
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Clear text safe zones at the bottom of the frame for model height and size callouts. Zero unnatural body warping.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO & SIZING REASSURANCE]: Commercial e-commerce fit guide photograph of diverse models standing side-by-side wearing [HERO PRODUCT: Garment type, color, fabric] in their respective sizes, occupying 80% of total frame width.
[BLOCK 2: OPTICAL RIG]: 50mm prime portrait lens at chest height, eye-level perspective, f/8 aperture keeping all models and garments in uniform sharpness.
[BLOCK 3: LIGHTING RIG]: Clean 5400K commercial catalog studio lighting with soft diffused shadows and crisp ground contact shadows.
[BLOCK 4: SUBORDINATE STAGE]: Seamless neutral light-gray studio cyclorama with zero props. Authentic models with natural relaxed postures and unretouched skin.
[BLOCK 5: EXCLUSIONS]: Negative: wax mannequins, uncanny valley, stiff identical poses, wooden barn, distorted hands, blurry garment hems.
```

---

### Archetype 08: The Curated Outfit Knolling (Wardrobe Pairing)
* **Universal Class:** The Specimen
* **Example Products (Illustrative Guidance, Not an Allowlist):** Chore jacket, tailored linen shirt, selvedge denim jeans, leather boots, technical daypack.
1. **Strategic Intent & Conversion Role:**  
   Cross-Sell & Complete the Look. Sells the entire lifestyle wardrobe by pairing the hero item with complementary pieces.
2. **The Physical Stage (The 95%):**  
   Top-down 90° flat-lay on a raw concrete, warm oak, or neutral linen floor. The hero jacket/shirt is laid out naturally alongside matching trousers, curated sneakers, sunglasses, and a watch.
3. **Optical & Lighting Directives:**  
   * *Lens:* 35mm Prime mounted overhead.  
   * *Aperture:* f/8 for uniform edge-to-edge sharpness.  
   * *Lighting:* Large overhead diffused softbox casting soft, subtle directional shadows.
4. **Sensory & Synesthetic Triggers:**  
   Styling clarity: how different fabric textures (cotton, wool, leather, acetate) harmonize together.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   The hero product must be the visual anchor (positioned centrally with 50% larger visual presence than accessories).
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO FLAT-LAY]: Overhead 90-degree knolling flat-lay photograph. In the center, [HERO PRODUCT: Garment, color, fabric] is laid out naturally, commanding 55% of the frame in tack-sharp focus.
[BLOCK 2: WARDROBE COMPLEMENTS]: Neatly arranged around the hero are 2-3 curated accessories [COMPLEMENTS: e.g., raw selvedge denim, leather boots, acetate sunglasses] creating a complete outfit.
[BLOCK 3: OPTICS]: 35mm prime lens overhead perpendicular to surface, f/8 aperture for edge-to-edge focal sharpness.
[BLOCK 4: STAGE & LIGHT]: Honed light-gray concrete or natural linen floor. Soft diffused overhead softbox casting delicate micro-shadows under every fold.
[BLOCK 5: EXCLUSIONS]: Negative: messy scattered clothes, wrinkly paper, accessories covering the hero product, harsh overhead glare.
```

---

### Archetype 09: Elasticity, Stretch & Tension Macro
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** High-rise performance leggings, 4-way stretch boardshorts, compression base layer, comfort-stretch denim.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 5 (Activewear, Denim & Performance Wear). Makes stretch, recovery, and opacity easy to inspect. Use the garment's supplied stretch and opacity data for claims rather than treating a generated frame as a test.
2. **The Physical Stage (The 95%):**  
   Macro close-up of hands stretching the waistband, legging fabric, or denim seam outward, showing the intended tension and recovery behavior. If recovery has not been observed, do not state that it is guaranteed.
3. **Optical & Lighting Directives:**  
   * *Lens:* 100mm Macro.  
   * *Aperture:* f/5.6.  
   * *Lighting:* Crisp side-light revealing the micro-elastane fibers under tension without flattening the fabric depth.
4. **Sensory & Synesthetic Triggers:**  
   Muscular support, softness, low restriction, and opacity can be communicated as sensory goals; “squat-proof,” maximum stretch, and zero restriction require a supplied specification or test.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Fabric must demonstrate realistic structural recovery; weave must not look artificially blurred or digitized.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO & ELASTICITY]: Macro physical tension photograph of [HERO PRODUCT: Fabric, knit type, color]. Two natural hands grip and stretch the fabric horizontally, demonstrating extreme 4-way stretch elasticity across 75% of the frame.
[BLOCK 2: OPACITY DETAIL]: Under the chosen, documented stretch condition, show the textile weave and opacity clearly. If the condition is illustrative, avoid “100% opaque,” “zero tears,” or other absolute performance wording.
[BLOCK 3: OPTICS]: 100mm True Macro lens, f/5.6 aperture, locking tack-sharp focus on taut elastane fibers and knit structure.
[BLOCK 4: STAGE & LIGHT]: Raking 5400K directional studio side-light revealing surface ribbing and elasticity; neutral studio background in soft bokeh.
[BLOCK 5: EXCLUSIONS]: Negative: see-through fabric, distorted fingers, blurry weave, synthetic plastic shine, domestic clutter.
```

---

### Archetype 10: The Luxury Unboxing & Hangtag Presentation
* **Universal Class:** The Specimen
* **Example Products (Illustrative Guidance, Not an Allowlist):** Cashmere crewneck, handmade leather shoes, bespoke suit jacket, luxury sunglasses case, silk robe.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 7 (Gifting & Delivery Confidence). Shows the intended unboxing experience. Packaging and fulfillment details must come from the actual delivery configuration.
2. **The Physical Stage (The 95%):**  
   Garment neatly folded inside custom embossed matte black box, wrapped in branded translucent glassine tissue paper, sealed with a foil sticker. Heavy cotton embossed hangtag resting on the collar.
3. **Optical & Lighting Directives:**  
   * *Lens:* 50mm Prime at 45° angle.  
   * *Aperture:* f/4.  
   * *Lighting:* Warm, low-contrast domestic morning light.
4. **Sensory & Synesthetic Triggers:**  
   The tactile pleasure of unwrapping luxury goods: the crinkle of fine tissue paper, the thick stock of letterpress paper tags, pristine fold lines.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Branded packaging details must match real fulfillment specifications.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO UNBOXING]: High-end unboxing delivery photograph of [HERO PRODUCT: Garment type, fabric, color] neatly folded inside an open luxury presentation box with embossed letterpress hangtag, occupying 75% of frame weight.
[BLOCK 2: OPTICAL RIG]: 50mm prime lens, 45-degree angle looking down into the open box, f/4 aperture for crisp packaging detail.
[BLOCK 3: LIGHTING RIG]: Soft 4800K natural window daylight illuminating the crisp tissue paper folds and letterpress cardstock texture with gentle contact shadows.
[BLOCK 4: SUBORDINATE STAGE]: Resting on a dark walnut tabletop. Background is clean and 2 stops darker in soft bokeh.
[BLOCK 5: EXCLUSIONS]: Negative: torn messy cardboard, crushed corners, plastic poly-bags, unreadable tags, cheap corrugated shipping boxes.
```

---

### Archetype 11: The 5-Year Break-In & Patina Evolution
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** Raw indigo selvedge jeans, full-grain leather boots, waxed canvas field coat, heavy wool knitwear, leather belt.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 6 & Brand Longevity Storytelling. Dismantles anxiety about fast fashion: *"Will this fall apart after a year, or will it mature?"* A documented wear study can support the claim; a generated later state is a material-aging illustration.
2. **The Physical Stage (The 95%):**  
   Rustic craftsman workbench or sunlit oak floorboards. A side-by-side: unworn piece on the left and a documented later sample or clearly labeled conceptual mature state on the right, displaying the intended wear pattern without inventing elapsed time.
3. **Optical & Lighting Directives:**  
   * *Lens:* 85mm Prime.  
   * *Aperture:* f/5.6 for uniform edge-to-edge sharpness across both pieces.  
   * *Lighting:* Raking directional side-daylight (4800K) casting crisp micro-shadows into creases, worn grain, and softened seam lines.
4. **Sensory & Synesthetic Triggers:**  
   Visceral tactile permanence: buttery softened grain, mold-to-body ergonomics, the emotional pride of enduring heirloom ownership.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Wear patterns must be biomechanically authentic (natural knee/elbow honeycombs), never artificial acid or laser washes.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO COMPARISON]: A side-by-side material longevity photograph of [HERO PRODUCT: Raw denim / leather / waxed canvas], occupying 80% of frame width on an aged timber workbench.
[BLOCK 2: COMPARISON GRAMMAR]: ON THE LEFT: The brand-new, unworn original garment. ON THE RIGHT (THE HERO): [DOCUMENTED LATER SAMPLE OR CLEARLY LABELED CONCEPTUAL MATURE STATE], showing the supplied patina, fade creases, softened grain, and stitching condition. Do not invent a five-year record.
[BLOCK 3: OPTICS]: 85mm prime lens, eye-level perspective, f/5.6 aperture keeping both pieces in tack-sharp focus.
[BLOCK 4: STAGE & LIGHT]: Raking 4500K workshop daylight casting dimensional shadows into fabric creases, soft contact floor shadows.
[BLOCK 5: EXCLUSIONS]: Negative: ripped ruined rags, cheap acid wash, artificial laser distress, synthetic plastic fabrics, messy tools.
```

---

### Archetype 12: The Dual-Model Real-World Fit & Layering Comparison
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** 3-layer technical mountain shell, wool overcoat, selvedge denim jacket, structured blazer, down parka.
1. **Strategic Intent & Conversion Role:**  
   PDP Sizing Guide & Slot 2/4 (Reduces Sizing & Layering Anxiety). Demonstrates drape differences between a base-layer fit and an over-layer fit using the organization's actual size chart and representative models.
2. **The Physical Stage (The 95%):**  
   Clean, seamless light-gray studio cyclorama with zero background clutter. Two real, athletic men/women standing naturally side-by-side with genuine, candid expressions and unretouched skin.
3. **Optical & Lighting Directives:**  
   * *Lens:* 75mm Prime portrait lens.  
   * *Aperture:* f/8 for tack-sharp focus across both jackets from collar to hem.  
   * *Lighting:* Clean 5500K commercial catalog studio lighting with soft contact floor shadows.
4. **Sensory & Synesthetic Triggers:**  
   Better fit understanding: seeing how sleeves fall, where shoulder seams sit, and how much chest room remains when worn alone versus layered. The image supports the size guide; it cannot represent every body.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Both models must stand in natural, relaxed postures with candid expressions. Avoid 4-model cramming that triggers wax-mannequin artifacts.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO & SIZING PROOF]: A commercial e-commerce sizing and fit comparison photograph of two athletic models standing naturally side-by-side on a clean, seamless light-gray studio cyclorama with zero background clutter. The garments occupy 80% of the visual frame.
[BLOCK 2: COMPARISON GRAMMAR]: ON THE LEFT: [MODEL A'S ACTUAL HEIGHT, BODY MEASUREMENTS, SIZE, AND BASE LAYER FROM THE SIZE GUIDE] wearing [HERO PRODUCT]. ON THE RIGHT: [MODEL B'S ACTUAL MEASUREMENTS, SIZE, AND LAYERING CONDITION]. Show sleeves, shoulder seams, and chest room clearly; avoid “zero binding” unless it is an observed result.
[BLOCK 3: OPTICS]: 75mm prime portrait lens, eye-level perspective, f/8 tack-sharp focus across both garments from collar to hem.
[BLOCK 4: STAGE & LIGHT]: Clean 5500K commercial catalog studio lighting with soft contact floor shadows, seamless neutral studio cyclorama with zero props.
[BLOCK 5: EXCLUSIONS]: Negative: wax figures, stiff mannequins, wooden barn, uncanny valley, CGI render, cartoon, four models, identical faces, distorted hands, stiff poses.
```

---

### Archetype 13: The Hydrostatic Deluge & DWR Water-Beading Stress Test
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** 3-layer technical shell, waterproof trail jacket, storm-proof poncho, all-weather commuting pants, alpine mountaineering shell.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 2 or Paid Social Performance Hook (Water-Resistance Demonstration). Makes the pain of a soaked garment and the relief of water beading visually immediate. Use the supplied hydrostatic/DWR test and conditions for any performance claim.
2. **The Physical Stage (The 95%):**  
   An authentic outdoor hiker or trail runner standing at a natural 3/4 profile in a driving misty alpine coastal rainstorm. The jacket fits anatomically and realistically with clean architectural lines and zero bizarre fabric extensions.
3. **Optical & Lighting Directives:**  
   * *Lens:* 85mm Prime.  
   * *Aperture:* f/2.8 for shallow subject separation.  
   * *Lighting:* Moody cinematic daylight sculpting the textile weave, with water droplets catching rim highlights.
4. **Sensory & Synesthetic Triggers:**  
   High-performance security: round water pearls sheeting cleanly off the DWR-treated membrane (lotus effect), storm hood cinched neatly, taped matte zippers bone-dry.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Inspect for cape-like or tentacle-like fabric hallucinations. Jacket should have a clean, anatomical silhouette at the intended crop.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO & DELUGE PROOF]: An editorial outdoor performance photograph of [HERO PRODUCT: 3-layer technical shell jacket, color, hood cinched]. The jacket is the undisputed hero, occupying 75% of the frame on an athletic model standing at a natural three-quarter profile in a misty coastal rainstorm.
[BLOCK 2: MEMBRANE PERFORMANCE]: Raindrops visibly bead, sheet, or soak according to the supplied DWR/membrane reference and test conditions. Keep the anatomical technical cut clean and realistic; if the output is an illustration, say so in the brief rather than presenting it as a pass/fail test.
[BLOCK 3: OPTICAL RIG]: 85mm prime lens, f/2.8 aperture, tack-sharp focus locked on the textured textile weave and water beading.
[BLOCK 4: SUBORDINATE STAGE]: Soft moody Pacific Northwest coastal mist and blurred pine trees in the background, 2.5 stops darker.
[BLOCK 5: EXCLUSIONS]: Negative: cape, capelet, flowing fabric behind back, extra limbs, awkward posture, CGI render, plastic skin, distorted anatomy, cartoon, bright sunny day, cluttered urban street.
```

---

### Archetype 14: The Ergonomic Range-of-Motion & Articulation Stress Test
* **Universal Class:** The Proof
* **Example Products (Illustrative Guidance, Not an Allowlist):** Climbing jacket, stretch commuter trousers, running shell, technical fleece, articulated work pants.
1. **Strategic Intent & Conversion Role:**  
   PDP Slot 4 (Kinetic Mobility De-risking). Eliminates the common customer fear: *"Will this bind across my back when I reach up, or pull out of my waist?"*
2. **The Physical Stage (The 95%):**  
   Modern climbing gym wall, outdoor boulder, or minimal studio stage. An athletic model captured in extreme physical articulation (reaching high overhead, deep high-knee step).
3. **Optical & Lighting Directives:**  
   * *Lens:* 50mm Prime.  
   * *Aperture:* f/5.6.  
   * *Lighting:* Clean, high-speed directional studio flash (1/2000s freeze) highlighting the underarm gussets, articulated elbow darts, and hemline stability.
4. **Sensory & Synesthetic Triggers:**  
   Show the intended range of motion: underarm gussets and articulated seams should remain legible during a representative movement. Do not imply zero restriction for all bodies from one pose.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   Underarm gussets and articulation darts must be clearly visible and in tack-sharp focus.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO & MOBILITY PROOF]: High-speed athletic action photograph of [HERO PRODUCT: Technical jacket / performance pants, color, articulated cut]. Model is captured mid-movement in full overhead arm reach or dynamic lunge, occupying 75% of frame weight.
[BLOCK 2: ERGONOMIC DETAIL]: The garment's underarm gusset and articulated elbow darts are visible under a representative extension, showing the observed hem and seam behavior. Use the supplied fit evidence for any promise about pulling or waist rise.
[BLOCK 3: OPTICS]: 50mm prime lens, eye-level perspective, f/5.6 aperture at 1/2000s shutter speed freezing movement without blur.
[BLOCK 4: STAGE & LIGHT]: Crisp 5500K directional rim lighting highlighting seam stretch and fabric flexion, neutral studio wall 2 stops darker.
[BLOCK 5: EXCLUSIONS]: Negative: motion blur, baggy bunched fabric, riding-up waistline, awkward yoga pose, distorted joints, cluttered gym background.
```

---

### Archetype 15: The Architectural Fashion Editorial Drape
* **Universal Class:** The Habitat
* **Example Products (Illustrative Guidance, Not an Allowlist):** Technical mountain parka, structured wool overcoat, cashmere trench, heavy twill overshirt, leather flight jacket.
1. **Strategic Intent & Conversion Role:**  
   Hero Header / Brand Lookbook (Luxury Price Ceiling Justification). Competes at the highest tier of international luxury editorial fashion (Arc'teryx Veilance, Prada Linea Rossa, Jil Sander).
2. **The Physical Stage (The 95%):**  
   Dramatic architectural setting: brutalist concrete museum steps, coastal granite cliff, or minimalist sculpture pavilion. Model standing in quiet, contemplative poise.
3. **Optical & Lighting Directives:**  
   * *Lens:* 85mm Prime.  
   * *Aperture:* f/2.8.  
   * *Lighting:* Moody, diffused overcast daylight or low blue-hour twilight with subtle architectural rim lighting sculpting the garment's structural silhouette.
4. **Sensory & Synesthetic Triggers:**  
   Pure architectural elegance: crisp collar stance, heavy drape, laser-cut hemlines, museum-grade textile dignity.
5. **Invariant Guardrails & Crop Safe-Zones:**  
   The garment occupies 75% of frame weight. Zero strange AI drapery artifacts or flapping capes.
* **Director's Rig Prompt Blueprint:**
```text
[BLOCK 1: HERO EDITORIAL]: Luxury architectural fashion editorial photograph of [HERO PRODUCT: Outerwear silhouette, fabric, color, hardware]. The garment is the undisputed hero, commanding 75% of frame height on an editorial model standing in poised profile.
[BLOCK 2: OPTICAL RIG]: 85mm prime lens, eye-level perspective, f/2.8 aperture for creamy cinematic subject separation.
[BLOCK 3: LIGHTING RIG]: Moody, diffused overcast 5600K daylight with soft directional fill sculpting the jacket's structural silhouette and taped matte hardware.
[BLOCK 4: SUBORDINATE STAGE]: Minimalist raw limestone or concrete architectural pavilion in the background, 2.5 stops darker in soft bokeh.
[BLOCK 5: EXCLUSIONS]: Negative: cheesy commercial poses, distorted fabric flaps, cape artifacts, ungrounded shadows, busy city streets, bright saturated colors.
```
