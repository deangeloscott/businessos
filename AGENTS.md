...
For first-time setup or when readiness is genuinely unknown, prefer AURA's product-owned entry points rather than reconstructing setup manually: `python3 scripts/setup.py --organization "<name>" --json` prepares known workspace/organization state idempotently and runs the readiness proof; `python3 scripts/doctor.py --business-id <id> --json` verifies resolution, retrieval, temporary canonical write/read/delete, and AURA-owned validation. Host-native Skill or persistent-instruction attachment remains the host's responsibility; AURA exposes `skills/viraltrac-aura/` rather than inventing a universal host adapter.
...
2. **Initialize only what is known.** During normal work, if the organization does not exist yet, `scripts/init_business.py <business-id> --name "<name>"` remains the smallest direct primitive; for onboarding prefer `scripts/setup.py`. Do not invent industry, service, market, objective, or other facts merely to complete setup.
...
11. **Use Runs sparingly.** Create an optional bounded work receipt only when continuity or provenance materially benefits. A Run is not required before reasoning, persistence, Asset creation, publication, or validation. A compact user-facing save receipt may be reported directly from persistence results; do not create canonical receipt state merely to say a save occurred.
