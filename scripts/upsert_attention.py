...
def upsert(business_id,key,attention_type,severity,title,reason,recommended_action=None,source_refs=None,evidence_refs=None,lineage=None,next_review_at=None,seen_at=None):
...
def main():
    parser=argparse.ArgumentParser(description='Create/update one deduplicated AURA AttentionItem. It records an organization-owned material condition needing awareness, not a permission, routing, or task gate.')
    parser.add_argument('business_id');parser.add_argument('--dedupe-key',required=True);parser.add_argument('--type',dest='attention_type',required=True);parser.add_argument('--severity',choices=SEVERITY,required=True);parser.add_argument('--title',required=True);parser.add_argument('--reason',required=True);parser.add_argument('--recommended-action');parser.add_argument('--source-ref',action='append',default=[]);parser.add_argument('--evidence-ref',action='append',default=[]);parser.add_argument('--lineage-ref',action='append',default=[]);parser.add_argument('--next-review-at');parser.add_argument('--seen-at');args=parser.parse_args()
    if not (ROOT/'instances'/args.business_id).exists():raise SystemExit(f'Unknown business: {args.business_id}')
    try:obj,path,created,reopened=upsert(args.business_id,args.dedupe_key,args.attention_type,args.severity,args.title,args.reason,args.recommended_action,args.source_ref,args.evidence_ref,args.lineage_ref,args.next_review_at,args.seen_at)
    except ValueError as exc:raise SystemExit(str(exc))
    print(json.dumps({'attention_id':obj['id'],'status':obj['status'],'created':created,'reopened':reopened,'occurrence_count':obj['occurrence_count'],'path':storage_ref(path)},indent=2))
