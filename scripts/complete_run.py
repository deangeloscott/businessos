...
A receipt preserves material continuity one-way. It may truthfully name a Playbook,
Workflow, external Skill, model-created method, or ad-hoc method. Completion does not
certify conformance, provider state, permissions, launch readiness, or business outcome.
"""
...
def complete_run(business_id,run_id,evidence=None,result_refs=None,decision_refs=None,summary=None,unresolved=None):
    ...
    method_type,method_ref=_normalize_method(run)
    if run.get('status')=='completed':return {'run_id':run_id,'status':'completed','category':'already_completed','method_type':method_type,'continuity':run.get('continuity') or {}}
    if run.get('status')!='active':raise ValueError(f"Run is not active: {run.get('status')}")

    evidence_refs=_refs(business_id,evidence);results=_refs(business_id,result_refs);decisions=_refs(business_id,decision_refs);unresolved_clean=[]
    for item in unresolved or []:
        text=str(item).strip()
        if text and text not in unresolved_clean:unresolved_clean.append(text)
    summary_text=str(summary).strip() if summary is not None else '';current=dict(run.get('continuity') or {})
    def merged(key,new):return list(dict.fromkeys([*(current.get(key) or []),*new]))
    final_evidence=merged('evidence_refs',evidence_refs);final_results=merged('result_refs',results);final_decisions=merged('decision_refs',decisions);final_unresolved=list(dict.fromkeys([*(current.get('unresolved') or []),*unresolved_clean]));final_summary=summary_text or current.get('summary')
    if not (final_summary or final_evidence or final_results or final_decisions or final_unresolved):raise ValueError('Run completion requires material organizational meaning: a summary, evidence/result/decision reference, or unresolved item')

    snapshot=rp.read_bytes()
    try:
        ts=now();run['method_type']=method_type;run['method_ref']=method_ref;run['status']='completed';run['updated_at']=ts
        run['continuity']={'format_version':'2.0','purpose':'organizational_work_receipt','state':'completed','method_type':method_type,'method_ref':method_ref,'summary':final_summary,'evidence_refs':final_evidence,'result_refs':final_results,'decision_refs':final_decisions,'unresolved':final_unresolved,'completed_at':ts}
        ...
        rp.write_bytes(snapshot);raise
    return {'run_id':run_id,'status':'completed','method_type':method_type,'method_ref':method_ref,'continuity':run['continuity'],'validation':{'errors':0,'warnings':warnings,'canonical_object_counts':counts},'rule':'This receipt references material continuity one-way. It does not mutate canonical results or certify Playbook/Workflow conformance, external execution, authorization, production readiness, or business outcomes.'}


def main():
    ap=argparse.ArgumentParser(description='Complete an optional AURA work receipt for any method.');ap.add_argument('business_id');ap.add_argument('run_id');ap.add_argument('--evidence',action='append',default=[]);ap.add_argument('--result',action='append',default=[]);ap.add_argument('--decision',action='append',default=[]);ap.add_argument('--summary');ap.add_argument('--unresolved',action='append',default=[]);a=ap.parse_args()
