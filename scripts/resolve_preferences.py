...
def _profile_applies(profile, system=None, contract=None, output_type=None, channel=None):
    a=profile.get('applies_to') or {}
    return (
        _matches_pattern(system, a.get('systems') or []) and
        _matches_pattern(contract, a.get('contracts') or []) and
...
def resolve_effective_preferences(business_id,operator_ref=None,team_ref=None,role_ref=None,system=None,contract=None,output_type=None,channel=None,task_preferences=None,fail_on_conflict=True):
    ...
    for profile,path in _load_profiles(business_id):
        ...
        if not _profile_applies(profile,system,contract,output_type,channel): continue
    ...
    result={
        ...
        'context':{'operator_ref':operator_ref,'team_ref':team_ref,'role_ref':role_ref,'system':system,'contract':contract,'output_type':output_type,'channel':channel},
        'precedence':['business','team','role','operator','task'],
        'mandatory_boundary':'BusinessOS invariants plus explicit business/Brand/compliance/approval/contract/task requirements override all preferences.',
...
def main():
    p=argparse.ArgumentParser(description='Resolve deterministic effective BusinessOS preferences. Preferences guide valid choices; they never override mandatory business/Brand/contract/approval requirements.')
    p.add_argument('business_id');p.add_argument('--operator-ref');p.add_argument('--team-ref');p.add_argument('--role-ref')
    p.add_argument('--system');p.add_argument('--contract');p.add_argument('--output-type');p.add_argument('--channel')
    p.add_argument('--task-preferences',help='JSON object file containing one-task optional preference overrides')
    p.add_argument('--run-id',help='Use operator/team/role/contract from an existing Run and write a run-local effective-preference snapshot')
    ...
    try:
        ...
        if a.run_id:
            ...
            result=resolve_effective_preferences(a.business_id,operator,team,role,a.system,a.contract,a.output_type,a.channel,task)
            out=None
