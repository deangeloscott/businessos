...
def main():
    p=argparse.ArgumentParser(description='Show what AURA is monitoring, how often, notification intent, what is due, pause state, and whether automation is actually active.')
    ...
    print(f"Tracked subjects: {data['tracked_subject_count']} | Due/unbound: {data['due_unbound_count']} | Due notices allowed: {data.get('proactive_due_notice_count',0)} | Mismatches: {data.get('configuration_mismatch_count',0)}")
    if not rows:
    ...
    for row in rows:
        ...
        if row.get('configuration_mismatch'):print('  attention: semantic watch and actual scheduler state disagree; reconcile the host scheduler before claiming the watch is paused/stopped')
    ...
    print('\nChange these through AURA in natural language (for example: “make pricing monthly but hiring weekly,” “only notify me on material changes,” “pause this watch,” or “stop automatic monitoring but keep what we learned”). Operators can inspect the same state with --json.')
