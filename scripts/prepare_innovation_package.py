...
def prepare_package(business_id,extension_id,detail=None,identity=None,evidence_summary=None,case_study=None,display_name=None,pseudonym=None,output=None):
    ...
    process={'mode':extension['mode'],'owner_system':extension['owner_system'],'target_contract_id':extension.get('target_contract_id'),'local_contract_id':extension.get('local_contract_id'),'title':extension['title'],'purpose':extension['purpose'],'discovery_terms':extension.get('discovery_terms') or [],'reads':extension.get('reads') or [],'writes':extension.get('writes') or [],'applies_when':extension.get('applies_when') or [],'does_not_apply_when':extension.get('does_not_apply_when') or [],'instructions':extension.get('instructions') or [],'verification':extension.get('verification') or [],'compatibility':extension.get('compatibility') or {'aura_min':os_version(),'aura_max':None}}
