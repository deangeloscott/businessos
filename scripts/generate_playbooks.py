#!/usr/bin/env python3
"""Generate plain-language human playbook navigation from canonical BusinessOS metadata.
...
PLAIN = {
    'core': {
        'name': 'BusinessOS Core',
        'summary': 'Figure out what the business needs, keep business facts organized, choose the right work, and coordinate work across BusinessOS.',
...
RESULT_OVERRIDES = {
    ...
    'core.coordination.multi-domain-request': 'Break one larger request into the right BusinessOS areas, put the work in the right order, and keep the handoffs clear.',
    ...
    'customer-optimization.journey.instrumentation': 'Set up tracking so BusinessOS can see how customers move from one important journey stage to another.',
    ...
    'industry.event.detect': 'Save an important industry event in the correct BusinessOS record so it can be verified, analyzed, and reused.',
...
GROUP_NAMES = {
    'action-control': 'Actions and approvals',
...
def write_root(registry, installed):
    ...
    lines = [
        '# What BusinessOS Can Do', '',
        'This is the plain-language guide to the work BusinessOS can help with.', '',
        ...
        f'This copy currently includes **{total} detailed playbooks** across the installed BusinessOS areas. The contracts and process maps remain the source of truth; this guide is only a simpler view of them.', '',
    ...
    lines += [
        ...
        'A playbook is a repeatable way for BusinessOS to do a specific job. A playbook can tell the AI:', '',
        ...
        'BusinessOS can combine several playbooks when one business request needs more than one kind of work.', '',
    ]
    if 'customer-intelligence' in installed:
        lines += [
            ...
            'See a plain-language example of how BusinessOS can find reviews, preserve the source text and screenshots when useful and allowed, remove duplicates, analyze themes and customer language, save reusable evidence, and route useful findings to the installed BusinessOS areas that can use them.', '',
        ]
    lines += [
        '## For advanced users', '',
