...
def upsert(args):
    ...
    if path.exists():
        ...
        obj = {
            ...
            "discovery_reason":None,"commercial_context":None,"fact_type_assessments":[],
            "last_checked_at":None,"next_check_at":None,"extensions":{"external_learning":{"outcome_events":[]}}
        }
...
def main():
    p=argparse.ArgumentParser(description="Create/update a SourceProfile. Source history changes discovery attention only; it never proves a current claim.")
    ...
    print(json.dumps({"path":str(path.relative_to(ROOT)),"id":obj["id"],"watch_status":obj["watch_status"],"attention_priority":obj["attention_priority"]},indent=2))
