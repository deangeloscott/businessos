...
def adopt_extension(business_id,proposal_id):
    ...
    obj={
        ...
        'created_at':existing.get('created_at') or ts,'updated_at':ts,'extension_version':existing.get('extension_version') or '1.0.0',
        'mode':mode,'owner_system':proposal['owner_system'],'target_contract_id':proposal.get('target_contract_id'),
        'local_contract_id':proposal.get('proposed_local_contract_id'),'title':proposal['title'],'purpose':proposal['summary'],
        'route_terms':proposal.get('route_terms') or [],'status':'active','scope':'business','scope_ref':None,'priority':100,
        'applies_when':proposal.get('applies_when') or [],'does_not_apply_when':proposal.get('does_not_apply_when') or [],
        'reads':proposal.get('reads') or [],'writes':proposal.get('writes') or [],
        'required_capabilities':proposal.get('required_capabilities') or [],'optional_capabilities':proposal.get('optional_capabilities') or [],
        'instructions':proposal.get('instructions') or [],'verification':proposal.get('verification') or [],
        'source_kind':'learning_evolved','source_learning_refs':proposal.get('learning_refs') or [],
        'source_refs':proposal.get('evidence_refs') or [],'evidence_refs':proposal.get('evidence_refs') or [],
        'compatibility':{'aura_min':os_version(),'aura_max':None},
        'extensions':{'playbook_evolution_proposal_ref':proposal_id}
    }
    _validate(obj);tmp=path.with_suffix('.tmp');tmp.write_text(json.dumps(obj,indent=2)+'\n');os.replace(tmp,path)
    proposal['status']='adopted';proposal['updated_at']=ts;ppath.write_text(json.dumps(proposal,indent=2)+'\n');return obj,path

def main():
    ap=argparse.ArgumentParser(description='Adopt evidence-backed Learning as a reversible business-scoped ProcessExtension. Explicit organization-authored SOPs use persist_process_extension.py instead of fabricating Learning.');ap.add_argument('business_id');ap.add_argument('proposal_id');a=ap.parse_args()
