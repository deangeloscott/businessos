...
def _migrate_contracts(dry_run):
    ...
    for path in list(contract_files()):
        ...
        if 'subcontracts' in meta:
            existing=meta.pop('subcontracts') or {};current=meta.get('workflows') or {}
            merged={}
        ...
        # Canonical reads/writes name the durable proposal type and must follow the semantic rename.
        meta['reads']=['WorkflowEvolutionProposal' if x=='PlaybookEvolutionProposal' else x for x in meta.get('reads',[])]
        meta['writes']=['WorkflowEvolutionProposal' if x=='PlaybookEvolutionProposal' else x for x in meta.get('writes',[])]
        if dirty:
...
def _migrate_process_maps(dry_run):
    ...
    for path in paths:
        ...
        for activity in data.get('activities',[]):
            ...
            if 'supporting_contracts' in activity:
                activity['supporting_workflows']=activity.pop('supporting_contracts')
...
def _migrate_instance_state(dry_run):
    changed=0;root=ROOT/'instances'
    if not root.exists():return changed
    for path in sorted(root.glob('*/**/*.json')):
        try:obj=json.loads(path.read_text(encoding='utf-8'))
        except Exception:continue
        if not isinstance(obj,dict):continue
        dirty=False;typ=obj.get('object_type')
        if typ=='ProcessExtension':
            if obj.get('mode')=='augment_contract':obj['mode']='augment_workflow';dirty=True
            if obj.get('mode')=='local_playbook':obj['mode']='local_workflow';dirty=True
            for key in ('required_capabilities','optional_capabilities'):
                if key in obj:obj.pop(key,None);dirty=True
            ext=obj.get('extensions') if isinstance(obj.get('extensions'),dict) else {}
            if 'playbook_evolution_proposal_ref' in ext:
                ext['workflow_evolution_proposal_ref']=ext.pop('playbook_evolution_proposal_ref');obj['extensions']=ext;dirty=True
        elif typ=='PlaybookEvolutionProposal':
            obj['object_type']='WorkflowEvolutionProposal';dirty=True
            if obj.get('change_kind')=='new_local_playbook':obj['change_kind']='new_local_workflow';dirty=True
            for key in ('required_capabilities','optional_capabilities'):
                if key in obj:obj.pop(key,None);dirty=True
        if dirty:changed+=1;_write_json(path,obj,dry_run)
    return changed
...
def migrate(dry_run=False):
    changed,retyped,capability_blocks,composition_blocks,body_changes=_migrate_contracts(dry_run);maps=_migrate_process_maps(dry_run);state=_migrate_instance_state(dry_run);removed=_retire_files(dry_run)
    return {'changed_workflow_files':len(changed),'retyped_playbook_metadata':retyped,'removed_capability_blocks':capability_blocks,'renamed_composition_blocks':composition_blocks,'workflow_body_term_updates':body_changes,'process_maps_updated':maps,'organization_state_objects_migrated':state,'retired_files':removed,'dry_run':dry_run}


def main():
    p=argparse.ArgumentParser(description='Apply the AURA Playbook → Workflow semantic migration and retire the invented capability vocabulary.');p.add_argument('--dry-run',action='store_true');a=p.parse_args();result=migrate(a.dry_run)
