...
def _args(byid):
    raw=sys.argv[1:]
    # v0.1.x convenience: create_run.py BUSINESS_ID WORKFLOW_ID TASK
    ...
    p.add_argument('--contract-id',help=argparse.SUPPRESS)
...
def main():
    ...
    workflow_id=a.workflow_id or a.contract_id
    workflow=byid.get(workflow_id) if workflow_id else None
    if workflow_id and not workflow:raise SystemExit('Unknown AURA Workflow')
    if workflow and workflow.get('type')!='workflow':raise SystemExit(f'{workflow_id} is not an AURA Workflow')
    ...
    if playbook:
        ...
        method_type='aura_workflow';method_ref=workflow_id
    ...
    preference_method=workflow_id or (playbook.get('entry_workflow') if playbook else None)
    ...
    obj={
        ...
        'method_type':method_type,'method_ref':method_ref,'playbook_id':playbook['id'] if playbook else None,'contract_id':workflow_id,
