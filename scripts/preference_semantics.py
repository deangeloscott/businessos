...
permission/approval store or silently convert a one-task authorization boundary
into reusable operator/business state.
"""
import copy
import hashlib
import json
...
# Key names that explicitly model permission/approval state rather than preference.
...
def forbidden_preference_key(key):
    """Return True when a key itself represents permission/approval state."""
    nk=_norm_key(key)
...
def forbidden_preference_text(value):
    """Return True when a string expresses an action authorization boundary."""
    if not isinstance(value,str):
        return False
    text=' '.join(value.split())
    return any(pat.search(text) for pat in TEXT_PATTERNS)


def preference_semantic_errors(preferences, prefix='preferences'):
    """Return path-qualified errors for permission/approval semantics inside preferences."""
    errors=[]
    def walk(v,path):
        if isinstance(v,dict):
            for k,x in v.items():
                if forbidden_preference_key(k):
                    errors.append(f'{path}.{k}: authorization/approval state is not a PreferenceProfile value')
                walk(x,f'{path}.{k}')
        elif isinstance(v,list):
            for i,x in enumerate(v): walk(x,f'{path}[{i}]')
        elif isinstance(v,str):
            text=' '.join(v.split())
            if forbidden_preference_text(text):
                errors.append(f'{path}: authorization/approval boundary must not be stored as a preference: {text!r}')
    if not isinstance(preferences,dict):
        return [f'{prefix}: preferences must be an object']
...
def sanitize_legacy_preferences(preferences, prefix='preferences'):
    """Remove only values that RC17 semantics already reject.

    Returns ``(cleaned_preferences, removals)``. Each removal records a path,
    reason, and SHA-256 fingerprint of the removed value without persisting the
    old authorization content itself. This is intended for explicit legacy-state
    migration, not for silently accepting new invalid PreferenceProfiles.
    """
    if not isinstance(preferences,dict):
        raise ValueError(f'{prefix}: preferences must be an object')

    DROP=object()
    removals=[]

    def record(path,reason,value):
        removals.append({'path':path,'reason':reason,'value_sha256':_fingerprint(value)})
    ...
    def clean(value,path):
        if isinstance(value,dict):
            ...
            for key,item in value.items():
                ...
                if forbidden_preference_key(key):
                    record(child,'authorization_key',item)
                    changed=True
                    continue
                ...
                if cleaned != item:
                    changed=True
            if changed and not out and path != prefix:
                return DROP
            return out
        if isinstance(value,list):
            ...
            if changed and not out:
                return DROP
            return out
        if isinstance(value,str) and forbidden_preference_text(value):
            record(path,'authorization_text',value)
            return DROP
    ...
    if cleaned is DROP:
        cleaned={}
    # Fail closed if the sanitizer ever drifts from the validator.
    residual=preference_semantic_errors(cleaned,prefix)
    if residual:
        raise ValueError('Legacy preference cleanup left invalid authorization semantics: '+'; '.join(residual[:10]))
...
def validate_preference_semantics(preferences, prefix='preferences'):
    ...
    if errors:
        raise ValueError(
            '; '.join(errors[:10]) +
            '. Keep reusable style/work-method choices in PreferenceProfile; keep current task/action boundaries in the user request/Run, and persist explicit formal authorization as Approval when applicable.'
        )
    return preferences
