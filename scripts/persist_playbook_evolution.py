#!/usr/bin/env python3
"""Persist evidence-backed proposals for reusable organization operating knowledge.

The historical object name PlaybookEvolutionProposal remains for v0.1.x state compatibility,
but proposals may now improve installed Workflows or define a new organization-local
Workflow. Tool/provider capability vocabularies are intentionally not part of the proposal.
"""
from _common import *
...
def persist_proposal(business_id,payload):
    ...
    if kind=='augment_existing' and (not target or not _canonical_contract_exists(target)):raise ValueError(f'augment_existing requires an installed canonical target workflow: {target!r}')
    if kind in {'new_local_workflow','new_local_playbook'}:
        kind='new_local_workflow'
    ...
    obj={
        ...
        'target_contract_id':target,'proposed_local_contract_id':local_id,'title':payload.get('title') or 'Operating knowledge evolution proposal','summary':payload.get('summary'),
        ...
        'status':existing.get('status') if existing.get('status')=='adopted' else 'candidate','extensions':payload.get('extensions') or {}
    }
...
def main():
    parser=argparse.ArgumentParser(description='Persist a bounded evidence-backed proposal to improve an AURA Workflow or create organization-local Workflow knowledge.');parser.add_argument('business_id');parser.add_argument('--proposal-file',required=True);args=parser.parse_args()
