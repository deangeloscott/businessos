...
Use this when the organization intentionally defines reusable SOP/instructions. Do not
fabricate Learning merely to make organization-authored process knowledge recordable.
"""
...
def _validate_method_metadata(reads,writes,required_capabilities,optional_capabilities):
    ...
    catalog=json.loads((ROOT/'core/capabilities/catalog.json').read_text());valid_caps={row.get('id') for row in catalog.get('capabilities',[]) if row.get('id')}
    unknown_caps=sorted((set(required_capabilities)|set(optional_capabilities))-valid_caps)
    if unknown_caps:raise ValueError('Unknown provider-neutral capability ID(s): '+', '.join(unknown_caps))
...
def persist_extension(business_id,spec):
    ...
    mode=spec.get('mode') or 'local_playbook';owner=spec.get('owner_system') or 'core';source_refs=_validate_sources(bid,spec.get('source_refs'));scope=spec.get('scope') or 'business';scope_ref=_validate_scope(scope,spec.get('scope_ref'))
    target=spec.get('target_contract_id');local_id=spec.get('local_contract_id')
    if mode=='augment_contract':
        if not _canonical_contract(target):raise ValueError(f'augment_contract requires an installed target_contract_id: {target!r}')
        local_id=None
    ...
    reads=list(dict.fromkeys(spec.get('reads') or []));writes=list(dict.fromkeys(spec.get('writes') or []));required_capabilities=list(dict.fromkeys(spec.get('required_capabilities') or []));optional_capabilities=list(dict.fromkeys(spec.get('optional_capabilities') or []))
    _validate_method_metadata(reads,writes,required_capabilities,optional_capabilities)
...
def main():
    parser=argparse.ArgumentParser(description='Persist explicit organization-authored local SOP/instructions as AURA ProcessExtension without fabricating Learning.')
