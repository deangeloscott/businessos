#!/usr/bin/env python3
"""Persist bounded research evidence without requiring an AURA Playbook, Workflow, or Run."""
from _common import *
...
def _base(typ,oid,bid,ts):return {'id':oid,'object_type':typ,'schema_version':'1.0.0','business_id':bid,'created_at':ts,'updated_at':ts,'lineage':[]}
...
def _source_provenance(item,ref,acquisition,snapshot_path):
    ...
    workspace_supplied=_workspace_evidence_path(ref) or _workspace_evidence_path(snapshot_path);ref_is_http=ref.lower().startswith(('http://','https://'))
    ...
    declared_public_type=supplied.get('source_type') in PUBLIC_NARRATIVE_TYPES and ref_is_http
    ...
    missing=sorted({'source_type','origin','access_scope'}-set(supplied));raise ValueError(f'cannot determine source provenance mechanically for {ref!r}; specify {", ".join(missing)}. AURA will not default ambiguous evidence to public web.')
...
def _source(bid,item,workflow_id,method_type,method_ref,ts):
    ...
    ch='sha256:'+hashlib.sha256(content_basis.encode('utf-8')).hexdigest() if content_basis else None;sid=item.get('id') or _id('src',f'{ref}:{ch or pointer or "pointer"}')
    asset_refs=[];asset_objs=[]
    if snap:
        aid,loc=_copy_snapshot(bid,sid,snap);asset_refs.append(aid);a=_base('Asset',aid,bid,ts);ext=_method_extensions(workflow_id,method_type,method_ref);ext['source_ref']=sid
        a.update({'asset_type':'evidence_snapshot','business_role':'source evidence preservation','location_reference':loc,'version':'1','status':'active','extensions':ext});_validate('Asset',a);asset_objs.append(a)
    if text or payload is not None or asset_refs:status='captured';method='mixed' if sum(bool(x) for x in [text,payload is not None,asset_refs])>1 else ('text_excerpt' if text else ('api_record' if payload is not None else 'snapshot'))
    elif pointer:status='external_pointer';method='external_pointer'
    else:status='pointer_only';method='reference_only'
    ev={'capture_status':status,'capture_method':item.get('capture_method') or method,'acquisition_method':acquisition,'acquisition_reference':item.get('acquisition_reference'),'captured_text':text,'title':item.get('title'),'author_label':item.get('author_label'),'rating':item.get('rating'),'context':item.get('context'),'asset_refs':asset_refs,'evidence_pointer':pointer,'record_payload':payload,'capture_notes':item.get('capture_notes'),'provenance_resolution':provenance['resolution']};ev={k:v for k,v in ev.items() if v is not None and v!=[]}
...
def persist(bid,bundle):
    ...
    workflow_id=str(bundle.get('workflow_id') or '').strip() or None;reg=load_registry();match=next((c for c in reg['workflows'] if c.get('id')==workflow_id),None) if workflow_id else None
    if workflow_id and not match:raise ValueError(f'Unknown workflow_id: {workflow_id}')
    method_type=str(bundle.get('method_type') or '').strip() or ('aura_workflow' if workflow_id else None);method_ref=str(bundle.get('method_ref') or '').strip() or workflow_id
    if method_type and method_type not in {'aura_playbook','aura_workflow','external_skill','model_created','ad_hoc'}:raise ValueError(f'Unknown method_type: {method_type}')
    domain=bundle.get('domain') or (match.get('owner_system') if match else None) or 'core'
    if domain not in DOMAINS:raise ValueError(f'Unknown semantic domain: {domain}')
    seed=workflow_id or method_ref or method_type or 'research';ts=now();written=[]

    src_objs=[];asset_objs=[]
    for item in bundle.get('sources',[]):
        s,assets=_source(bid,dict(item),workflow_id,method_type,method_ref,ts);src_objs.append(s);asset_objs.extend(assets)
    ...
    for o in obs_objs:
        for ref in o['source_refs']:
            ...
            if s and is_public_source(s):
                ev=s.get('extensions',{}).get('businessos_evidence',{});acquisition=ev.get('acquisition_method') or 'unknown';status=ev.get('capture_status');adequate=(status=='captured' and acquisition in DIRECT_ACQUISITION_METHODS and (ev.get('captured_text') or ev.get('asset_refs') or ev.get('record_payload') is not None)) or (status=='external_pointer' and ev.get('evidence_pointer') and acquisition in AUTHORITATIVE_POINTER_METHODS)
