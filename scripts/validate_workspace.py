...
inventories, schedulers, orchestration, and other host-runtime concerns are intentionally
outside AURA.
"""
from _common import *
import json,re
...
def main():
    ...
    for p in contract_files():
        ...
        if ctype=='playbook':errors.append(f'{rel}: flattened playbook metadata remains; detailed contract procedures are Workflows and high-level Playbooks are generated separately')
    ...
    required_core=[
        'CONTEXT.md','AGENTS.md','AURA-ATTACHMENT.md','skills/viraltrac-aura/SKILL.md','docs/operating-knowledge.md','core/DEFAULTS.md','core/policies/agent-execution.md',
        'core/policies/active-business-truth.md','core/policies/evidence.md','core/policies/provenance.md','core/policies/preferences-and-adaptation.md','core/policies/business-isolation.md',
        'core/policies/context-provenance-and-claims.md','core/policies/monitoring-continuity.md','core/schemas/context/preference-profile.schema.json','core/schemas/decision/decision-record.schema.json',
        'scripts/enter.py','scripts/find_playbooks.py','scripts/find_workflows.py','scripts/remember.py','scripts/create_run.py','scripts/complete_run.py','scripts/canonical_store.py',
        'scripts/validate_business.py','scripts/resolve_contract.py','scripts/bootstrap_explicit_context.py','scripts/resolve_preferences.py','scripts/upsert_preference_profile.py','BEGINNERS-GUIDE.md'
    ]
    for rel in required_core:
        if not (ROOT/rel).exists():errors.append(f'missing AURA core component {rel}')

    retired_paths=[
        'core/capabilities/catalog.json','docs/adding-a-capability.md','generated/capability-usage-index.json','generated/playbook-candidate-index.json','PLAYBOOK-INDEX.md',
        'scripts/run_lifecycle.py','scripts/reconcile_runs.py','scripts/run_provenance.py','scripts/persist_run_results.py','scripts/finalize_run.py','scripts/finalize_work_receipt.py','scripts/finalize_sop_run.py','scripts/complete_sop_run.py','scripts/record_contract_completion.py','scripts/route_task.py','scripts/route_and_resolve.py','templates/manual-action.md','core/quality/action-quality.md'
    ]
    ...
    for item in errors[:180]:print('ERROR',item)
