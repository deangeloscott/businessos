...
RETIRED_PATHS=[
    ...
    'core/policies/playbook-evolution.md',
    'core/schemas/learning/workflow-evolution-proposal.schema.json','scripts/persist_workflow_evolution.py',
    'scripts/adopt_process_extension.py','core/workflows/learning/adopt-process-extension/CONTEXT.md',
    'scripts/run_lifecycle.py','scripts/reconcile_runs.py','scripts/run_provenance.py',
    'scripts/persist_run_results.py','scripts/finalize_run.py','scripts/finalize_work_receipt.py',
    'scripts/finalize_sop_run.py','scripts/complete_sop_run.py','scripts/record_contract_completion.py',
    'scripts/route_task.py','scripts/route_and_resolve.py','templates/manual-action.md',
    'core/quality/action-quality.md','core/contracts',
]
REQUIRED_CORE=[
    ...
    'scripts/persist_process_extension.py','scripts/process_extensions.py','scripts/validate_business.py',
    'scripts/resolve_workflow.py','scripts/bootstrap_explicit_context.py','scripts/resolve_preferences.py',
    'scripts/upsert_preference_profile.py','BEGINNERS-GUIDE.md',
]
...
def _validate_workflows(errors,owners,schema_titles):
    ...
    for path in workflow_files():
        ...
        for selector in meta.get('reads',[]):
            ...
            if isinstance(selector,dict) and set(selector)-{'type','owner_system','owner_scope'}:
                errors.append(f'{rel}: unsupported read selector keys {selector}')
...
def _validate_playbooks_and_maps(errors,ids,types):
    ...
    for map_path in map_paths:
        try:data=json.loads(map_path.read_text())
            ...
            errors.append(f'{map_path.relative_to(ROOT)} invalid JSON: {exc}');continue
...
def _validate_schemas(errors):
    ...
    for path in schemas():
        ...
        if schema.get('type')=='object' and schema.get('additionalProperties') is not False:
            errors.append(f'{path.relative_to(ROOT)}: top-level schema must be strict')
    return titles
...
def _validate_navigation(errors,installed):
    ...
    if 'customer-intelligence' in installed and not (ROOT/'docs/playbooks/examples/research-public-reviews.md').exists():
        errors.append('missing human example docs/playbooks/examples/research-public-reviews.md')
    ...
    for page in pages:
        ...
        try:text=page.read_text(encoding='utf-8')
            ...
            errors.append(f'{page.relative_to(ROOT)}: invalid UTF-8: {exc}');continue
...
def main():
    errors=[];warnings=[]
    declared=set(installation().get('installed_modules',[]))
    ...
    schema_titles=_validate_schemas(errors)
    ids,types=_validate_workflows(errors,owners,schema_titles)
    _validate_playbooks_and_maps(errors,ids,types)
    ...
    print(f'Workflows checked: {sum(1 for value in types.values() if value=="workflow")}')
