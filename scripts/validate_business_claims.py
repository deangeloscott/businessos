#!/usr/bin/env python3
from _common import *
from build_claim_manifest import scan_claims, business_name, is_candidate
import re
...
def _stem(tok):
    ...
    # normalize common doubled/orthographic forms enough for deterministic conservative matching
...
def _core_clause(text):
    # Benefits/explanations after these separators should not force an otherwise authorized core promise to match word-for-word.
    s=text or ''
...
def _is_general_business_claim(text,name):
    # Anything the customer-facing scanner deliberately surfaced is too business/promise-like
    # to escape as generic guidance. This keeps scanner and validator semantics aligned.
    return is_candidate(text,name)

def _substantive_support(sent,support_objs,name):
    """Conservative lexical authorization check.

    A trusted reference must support the substance of the customer-facing predicate, not merely exist.
    This intentionally favors narrowing/authorization over guessing semantic equivalence.
    """
    core=_core_clause(sent)
    stoks=_tokens(core,name)
    if not stoks:
        return True

    support_texts=[_support_text(o) for o in support_objs]
    all_support=set(_tokens(' '.join(support_texts),name))
    if not all_support:
        return False

    # Harmless framing around one or more actual approved claims can be supported by those claim objects.
    if 'commitment' in stoks and any(o.get('object_type')=='BusinessClaim' for o in support_objs):
        return True

    # Action/capability statements require the action-object substance to be present in support.
    action_pos=next((i for i,t in enumerate(stoks) if t in ACTION_VERBS),None)
    if action_pos is not None:
        action=stoks[action_pos]
        # CTA verbs such as contact/request/schedule may point at an authorized service whose noun form
        # is also in ACTION_VERBS (for example repair). Preserve that service token as predicate substance.
        if action in {'contact','request','schedule'}:
            objs=[t for t in stoks[action_pos+1:] if t not in GENERIC_RECIPIENT]
        else:
            objs=[t for t in stoks[action_pos+1:] if t not in ACTION_VERBS and t not in GENERIC_RECIPIENT]
        overlap=[t for t in objs if t in all_support]
        # Require broad coverage of the asserted object(s); this blocks attaching an unrelated trusted identity/market ref.
        if objs:
            ratio=len(overlap)/len(set(objs))
            if ratio < 0.60 and len(set(overlap)) < 2:
                return False
        # If the action verb itself is novel, require at least two supported object anchors to establish a conservative paraphrase.
        if action not in all_support and len(set(overlap)) < 2:
            return False
        return True

    # Non-action identity/descriptive claims need meaningful overlap; business-name presence alone is not enough.
...
def validate_manifest_sentences(manifest,sentences,idx,name,rel):
    """Validate classifications/support for the exact customer-facing statements supplied."""
    errors=[]
    if not isinstance(manifest,list):
        return [f'{rel} requires a claim manifest list; run the supported claim scanner and classify every candidate']
    bytext={_norm(x.get('text')):x for x in manifest if isinstance(x,dict) and x.get('text')}
    for sent in sentences:
        m=bytext.get(_norm(sent))
        if not m:
            errors.append(f'{rel} claim manifest missing artifact statement: {sent!r}')
            continue
        cls=m.get('classification')
        if cls not in ALLOWED:
            errors.append(f'{rel} claim {sent!r} has invalid classification {cls!r}')
            continue
        if cls=='placeholder':
        ...
        if support_objs and not _substantive_support(sent,support_objs,name):
            errors.append(f'{rel} approved business claim support_refs do not substantively authorize the customer-facing predicate: {sent!r}')
        corpus=' '.join(_support_text(o) for o in support_objs).lower()
        for pat in ESCALATION:
            hit=re.search(pat,sent.lower())
            if hit and not re.search(pat,corpus):
                errors.append(f'{rel} claim enlarges supported promise with unsupported term/phrase {hit.group(0)!r}: {sent!r}')
    return errors
...
def claim_errors(business_id,objects=None):
    ...
    for asset,path in objects:
        ...
        if not fp.exists() or fp.suffix.lower() not in {'.html','.htm','.md','.txt'}:continue
        bos=(asset.get('extensions') or {}).get('businessos',{}) if isinstance(asset.get('extensions'),dict) else {}
        # Only externally/user-facing files need this control. Marketing assets default customer-facing; content assets opt out explicitly with customer_facing=false.
        ...
        if not isinstance(manifest,list):
            ...
            continue
        candidates=scan_claims(business_id,fp)
