...
CORE_HINTS=[
 (r'\b(revenue|sales|profit|growth|conversion|marketing).*(not growing|isn.t growing|is not growing|not working|flat|stagn|declin|down|dropped|drop).*(why|wrong|figure|diagnos|stuck)?|\b(why|figure out|diagnos|what.*holding back|where.*stuck).*(revenue|sales|profit|growth|business|marketing)|\bfigure out what( is|.s) wrong\b|\b(lots|plenty|more).*(traffic|leads).*(few|no|less).*(customers?|sales|revenue)', 'core.diagnosis.business-problem'),
 (r'\b(what should (we|i) (work on|do|focus on)|what (should|do) (we|i) do next|where should (we|i) start|what.*work on first|next best (work|thing)|highest[- ]value (opportunit|thing)|biggest (growth )?opportunit|best opportunit|what.*focus on.*(quarter|month|year)).*', 'core.opportunity.discover-next-best-work'),
