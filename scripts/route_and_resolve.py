...
FEATURE_HINTS=[
 ...
 (r'\b(?:pause|resume|stop|disable|enable|change|update|adjust|make|set).*(?:watch|monitoring|monitor|tracked subject|recurring check|cadence|notification)|\b(?:notify|notification).*(?:material change|every check|all checks|silent|quiet|only)|\b(?:daily|weekly|monthly|quarterly|every\s+\d+\s+(?:day|days|week|weeks|month|months)).*(?:watch|monitor|check|pricing|hiring|news|content)', 'core.intelligence.subject-monitoring'),
