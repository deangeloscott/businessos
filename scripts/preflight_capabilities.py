...
def preflight(business_id,contract_id,environment=None,include_optional=False,use_declared_environment=False):
    ...
    if not discovery_complete and not use_declared_environment:return {'version':os_version(),'business_id':business_id,'contract_id':contract_id,'environment':environment,'status':'host_discovery_required','automated_ready':False,'required':[],'optional':[],'optional_checked':include_optional,'process_extension_ids':extension_ids,'host_discovery':{'completed':False,'policy':'core/policies/host-capability-discovery.md'},'recommendations':_recommendations(required+optional),'next_action':'Inspect current host tools, map clear capabilities, run scripts/bootstrap_environment.py, then rerun preflight. If discovery is impossible, rerun with --use-declared-environment. Trusted local capability packs may then be inspected for still-missing capabilities.','rule':'ProcessExtension requirements are additive; provider/tool choice remains separate from the business process.'}
    def check(cap):
        ...
        elif r['status']=='local_pack_check_required':row['execution_state']='local_capability_discovery';row['decision_required']=False;row['next_action']='inspect_trusted_local_capability_pack';row['pack']=r.get('pack')
    ...
    elif any(x['execution_state']=='local_capability_discovery' for x in req):status='local_capability_discovery_required'
    elif any(x['execution_state']=='provider_decision' for x in req):status='decision_required'
    else:status='ready_with_fallback'
    return {'version':os_version(),'business_id':business_id,'contract_id':contract_id,'environment':environment,'status':status,'automated_ready':all(x['execution_state']=='available' for x in req),'required':req,'optional':opt,'optional_checked':include_optional,'process_extension_ids':extension_ids,'host_discovery':{'completed':discovery_complete,'profile':profile},'recommendations':_recommendations(required+optional),'rule':'Missing automation changes the executor, not the required process. Existing tools win; trusted local capability packs may be inspected before external acquisition; any system install/update/repair or new provider connection requires authorization.'}
