...
def upsert(business_id,name,scope,subject_ref,preferences,profile_id=None,priority=0,source_kind='explicit_user',source_refs=None,systems=None,contracts=None,output_types=None,channels=None,notes=None):
    ...
    applies={}
    for key,val in [('systems',systems),('contracts',contracts),('output_types',output_types),('channels',channels)]:
...
def main():
    p=argparse.ArgumentParser(description='Create/update one business-scoped PreferenceProfile. Preferences customize valid choices but do not override mandatory rules or authorize actions.')
    ...
    p.add_argument('--source-ref',action='append',default=[]);p.add_argument('--system',action='append',default=[]);p.add_argument('--contract',action='append',default=[]);p.add_argument('--output-type',action='append',default=[]);p.add_argument('--channel',action='append',default=[]);p.add_argument('--notes')
    a=p.parse_args()
    try:
        prefs=_load_preferences(a.preferences_file)
        path,obj=upsert(a.business_id,a.name,a.scope,a.subject_ref,prefs,a.id,a.priority,a.source_kind,a.source_ref,a.system,a.contract,a.output_type,a.channel,a.notes)
