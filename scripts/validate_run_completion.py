#!/usr/bin/env python3
"""Validate organization-owned Run continuity and conditional AURA playbook conformance.

Every bounded Run must tell the truth about what work happened and what durable results it
left behind. Contract/subcontract completion rules apply only when method_type is
`aura_playbook`; external Skills, model-created methods, and ad-hoc work are not forced to
pretend they executed an AURA contract.
"""
from _common import *
...
def _generic_run_errors(business_id,objects,contracts,active_run_id=None):
    """Validate durable Run linkage without assuming which method performed the work."""
    errors=[]
    for obj,path in objects:
        ...
        if origin in LEGACY_ORIGINS:
            ...
            continue
        if not rr:
            errors.append(f'{path} {typ} requires extensions.businessos.run_ref to preserve bounded organizational work provenance; only explicitly imported/preexisting state is exempt');continue
        ...
        if bos.get('run_method_type') and bos.get('run_method_type')!=method:
            errors.append(f'{path} {typ} run_method_type does not match referenced Run: {bos.get("run_method_type")!r} vs {method!r}')
        ...
        if not expected_active and str(Path(path)) not in _continuity_refs(r):
            errors.append(f'{path} {typ} is not indexed by its completed Run work receipt: {rr}')
        if method=='aura_playbook':
            ...
            if not root:errors.append(f'{path} {typ} Run root contract is missing from the installed registry: {root_id!r}')
                ...
                if owner and root_owner not in {None,'core',owner}:errors.append(f'{path} {typ} owner_system does not match producing AURA playbook owner: {owner} vs {root_owner}')
...
def _semantic_aura_run_errors(business_id,objects,contracts):
    """Re-check completion profiles only for Runs that actually claim AURA SOP completion."""
    errors=[];seen=set()
...
def _asset_run_errors(business_id,objects,contracts,active_run_id=None):
    ...
    for asset,path in objects:
        ...
        if origin in LEGACY_ORIGINS and rr:errors.append(f'{path} Asset cannot combine origin={origin!r} with run_ref; a draft generated in current work is Run-produced even when unpublished')
        ...
        if not rr:
            if origin not in LEGACY_ORIGINS:errors.append(f'{path} Content/Marketing Asset requires extensions.businessos.run_ref for durable production provenance; imported/preexisting Assets must explicitly mark origin')
            continue
        _,r,m,problem=_load_run(rr)
        ...
        if not expected_active and str(Path(path)) not in continuity_refs:
            errors.append(f'{path} production Asset is not indexed by its completed Run work receipt: {rr}')
        ...
        # Only a Run that claims an AURA playbook must satisfy AURA contract/subcontract QA.
