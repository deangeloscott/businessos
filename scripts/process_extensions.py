...
semantic applicability and whether to use them.
"""
from _common import *
import json,re
...
def local_playbook_candidates(task,business_id,team_ref=None,role_ref=None,operator_ref=None,top=5):
    ...
    if not q:return []
    words=set(re.findall(r'[a-z0-9]{2,}',q));rows=[]
...
def _extension_markdown(ext):
    lines=[f"### {ext.get('title')} (`{ext.get('id')}`)",'',f"Scope: `{ext.get('scope')}`. This is organization-owned operating knowledge, not authority over the active model/harness/user.",'']
...
def resolve_effective(contract_id,business_id,team_ref=None,role_ref=None,operator_ref=None):
    ...
    if exts:content+='\n\n## Active Organization Process Extensions\n\nThese organization-scoped extensions add relevant operating knowledge to this AURA playbook. They do not create runtime authority or prevent the active intelligence from choosing another valid method.\n\n'+'\n'.join(_extension_markdown(x) for x in exts)
