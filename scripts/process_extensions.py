...
ProcessExtensions are organization-owned operating knowledge. Deterministic code may
list/filter them and provide bounded lexical candidates; the active model/user decides
semantic applicability, conflict resolution, and whether to use them.
"""
from _common import *
import json,re
...
def active_extensions(business_id,team_ref=None,role_ref=None,operator_ref=None):
    ...
    # Ordering is presentation only. More-specific scopes appear later so their context is
    # easy for a model to notice; AURA does not turn order into semantic precedence.
...
def local_playbooks(business_id,team_ref=None,role_ref=None,operator_ref=None):return [extension for extension in active_extensions(business_id,team_ref,role_ref,operator_ref) if extension.get('mode')=='local_playbook']


def local_playbook_candidates(task,business_id,team_ref=None,role_ref=None,operator_ref=None,top=5):
    """Return bounded lexical candidates only; never claim semantic selection authority."""
...
def _merge_metadata(base_meta,extensions):
    meta=json.loads(json.dumps(base_meta));capabilities=meta.setdefault('capabilities',{});required=[x for x in capabilities.get('required',[]) if x!='none'];optional=[x for x in capabilities.get('optional',[]) if x!='none'];reads=[selector_type(x) for x in meta.get('reads',[])];writes=[selector_type(x) for x in meta.get('writes',[])]
    for extension in extensions:
        required+=extension.get('required_capabilities') or [];optional+=extension.get('optional_capabilities') or [];reads+=extension.get('reads') or [];writes+=extension.get('writes') or []
    required=list(dict.fromkeys(required));optional=[x for x in dict.fromkeys(optional) if x not in required];meta['capabilities']={'required':required or ['none'],'optional':optional or ['none']};meta['reads']=list(dict.fromkeys(reads));meta['writes']=list(dict.fromkeys(writes));meta['process_extension_ids']=[extension['id'] for extension in extensions];return meta


def _extension_markdown(extension):
    lines=[f"### {extension.get('title')} (`{extension.get('id')}`)",'',f"Scope: `{extension.get('scope')}`. This is organization-owned operating knowledge, not authority over the active model/harness/user.",'']
...
def resolve_effective(contract_id,business_id,team_ref=None,role_ref=None,operator_ref=None):
    local=next((extension for extension in local_playbooks(business_id,team_ref,role_ref,operator_ref) if extension.get('local_contract_id')==contract_id),None)
    if local:
        meta={'id':local['local_contract_id'],'type':'playbook','owner_system':local['owner_system'],'reads':local.get('reads') or [],'writes':local.get('writes') or [],'capabilities':{'required':local.get('required_capabilities') or ['none'],'optional':local.get('optional_capabilities') or ['none']},'process_extension_ids':[local['id']],'local_playbook':True}
        body=['---',f"id: {meta['id']}",'type: playbook',f"owner_system: {meta['owner_system']}",'---',f"# {local['title']}",'','## Purpose',local['purpose'],'','## Applicability']+[f"- Applies when: {item}" for item in local.get('applies_when') or []]+[f"- Does not apply when: {item}" for item in local.get('does_not_apply_when') or []]+['','## Process']+[f"{index+1}. {item}" for index,item in enumerate(local.get('instructions') or [])]+['','## Verification']+[f"- {item}" for item in local.get('verification') or []]+['','## Extension Boundary','This is organization-scoped operating knowledge. The active model/harness/user may choose or adapt another method; AURA truth, provenance, persistence, and organization-isolation integrity still apply.','']
        return None,meta,'\n'.join(body),[local]
    ...
    path,meta,_=canonical;extensions=[extension for extension in active_extensions(business_id,team_ref,role_ref,operator_ref) if extension.get('mode')=='augment_contract' and extension.get('target_contract_id')==contract_id];effective_meta=_merge_metadata(meta,extensions);content=path.read_text()
    if extensions:content+='\n\n## Active Organization Process Extensions\n\nThese organization-scoped extensions add relevant operating knowledge to this AURA playbook. They do not create runtime authority, numerical precedence, or prevent the active intelligence from choosing another valid method. If applicable extensions conflict semantically, the model/user resolves that conflict from the actual organization context.\n\n'+'\n'.join(_extension_markdown(extension) for extension in extensions)
...
def effective_capabilities(contract_id,business_id,team_ref=None,role_ref=None,operator_ref=None):
    _,meta,_,extensions=resolve_effective(contract_id,business_id,team_ref,role_ref,operator_ref);caps=meta.get('capabilities') or {};return {'required':[x for x in caps.get('required',[]) if x!='none'],'optional':[x for x in caps.get('optional',[]) if x!='none'],'process_extension_ids':[extension['id'] for extension in extensions]}
