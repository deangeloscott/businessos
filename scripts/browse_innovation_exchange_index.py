...
def browse(index_path,query=None,owner_system=None,detail_level=None):
    ...
    for e in idx['entries']:
        ...
        hay=' '.join([e['title'],e['purpose'],e['owner_system'],e.get('target_workflow_id') or '',e.get('local_workflow_id') or '']).lower();score=sum(1 for w in words if w in hay) if words else 0
