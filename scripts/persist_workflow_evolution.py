#!/usr/bin/env python3
"""Persist evidence-backed proposals for reusable organization Workflow knowledge."""
from _common import *
...
def persist_proposal(business_id,payload):
    ...
    seed='|'.join([business_id,str(payload.get('owner_system')),str(kind),str(target),str(local_id),str(payload.get('summary'))]);oid='wev_'+hashlib.sha256(seed.encode()).hexdigest()[:20]
    ...
    _validate('WorkflowEvolutionProposal',obj);temporary=path.with_suffix('.tmp');temporary.write_text(json.dumps(obj,indent=2)+'\n');os.replace(temporary,path);return obj,path
