...
9. [HYBRID] Persist traceable SourceRecords/Observations selected as durable evidence. Preserve potential proof candidates as ordinary findings/evidence so the capable model can use proof-analysis operating knowledge when relevant; do not emit runtime events or route them through an internal AURA dispatcher.
