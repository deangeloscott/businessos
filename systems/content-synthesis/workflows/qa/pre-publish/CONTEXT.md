...
2. [HYBRID] Confirm Brand, editorial, fact/claim, platform-native, accessibility, and originality QA have been satisfied where applicable. For customer-facing rendered media, inspect the actual final artifact—not only its source file, prompt, transcript, sidecar, or metadata. Every material product behavior, integration, timing, outcome, proof, or performance statement must be supported by trusted business/evidence state or removed/narrowed; conceptual or illustrative workflow content must be visibly framed as such instead of implying the business/product performs it.
3. [HYBRID] For opaque/rendered media, inspect the audience-visible/spoken/visual claims in the actual artifact directly. A compact `extensions.businessos.claim_surface_ref` sidecar may be preserved when it materially improves future handoff, auditability, or text-oriented review. When one exists, verify it resolves to the exact Asset and matches the actual render. Do not require a sidecar merely because the medium is opaque; it is optional continuity/provenance support, not semantic authority and never a substitute for inspecting the artifact itself.
