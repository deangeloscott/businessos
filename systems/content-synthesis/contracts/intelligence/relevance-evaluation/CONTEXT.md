...
## Purpose
Evaluate whether shared intelligence from another semantic domain materially affects audience communication and platform-native content without duplicating the upstream Insight or launching expensive work automatically.
...
3. [AI] Evaluate the domain mechanism: Ask whether the Insight creates an independent valuable communication need, changes an active content brief, or supplies evidence/examples for delegated production. Do not create a Content Opportunity when the only need is execution of another systems existing Opportunity.
