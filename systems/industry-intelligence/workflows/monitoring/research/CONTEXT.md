...
3. [HYBRID] Evaluate evidence strength, replication/independence, external validity, measurement quality, freshness, and applicability. Draw on the research-quality method when a study/report materially supports a decision or durable Insight; it is not a mandatory downstream Workflow. Publication status alone is not proof of truth or relevance.
...
## Completion Criteria
- The organization has the smallest useful current research evidence for the decision, with durable findings preserved only where future work benefits and no required Workflow-composition chain.
