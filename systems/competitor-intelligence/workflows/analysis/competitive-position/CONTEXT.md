...
workflows:
  ...
  - id: competitor.discovery.competitive-set
    when: The relevant competitive set is not already clear enough for the decision.
  - id: competitor.analysis.profiling
    when: Material competitors need deeper current profiles before comparison or synthesis.
  - id: competitor.analysis.benchmark
    when: A benchmark, strengths/weaknesses comparison, or relative performance frame would improve the decision.
  - id: competitor.analysis.competitive-implications
    when: The findings need to be translated into broader strategic or operating implications.
