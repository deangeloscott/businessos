...
6. [AI] Update the factual profile with current products/services, audiences, positioning, offers/pricing, public identities, and notable recent changes where evidence supports them. Preserve strategic interpretations—including strengths/weaknesses hypotheses and implications—as scoped Insights rather than embedding them as entity facts. Treat hiring, funding, partnerships, M&A, expansion, content/message shifts, or similar signals as evidence to interpret, not automatic proof of strategy. Where a material profile field is unsupported or stale, keep it limited/unknown instead of filling the field from memory or unrelated evidence.
7. [HYBRID] Link the profile to its supporting Observations and active Insights, preserve material contradictions/gaps in those evidence objects, and update `last_reviewed`. Do not manufacture a generic confidence score for the entity or identity match; use evidence plus the qualitative identity status where needed.
8. [DETERMINISTIC] Validate subject-scoped research evidence and persist the selected current profile state. Do not emit an AURA runtime event merely because the profile changed.
