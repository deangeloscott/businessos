...
13. [HYBRID] Use `competitor.analysis.competitive-implications` to route supported implications to the correct downstream domain owners without creating duplicate foreign-domain Opportunities or pretending that a competitive observation authorizes execution.
14. [DETERMINISTIC] Validate canonical state and `scripts/validate_research_evidence.py`. Do not claim decision-grade completion while a material conclusion outruns its subject-relevant evidence; deepen research, narrow/downgrade the conclusion, or leave the gap explicit. If an optional AURA playbook conformance receipt is being used, satisfy its contract-aware completion evidence before claiming that conformance. Preserve current evidence/snapshot dates and create/update SourceProfiles or refresh triggers only where future change could materially alter the decision.
