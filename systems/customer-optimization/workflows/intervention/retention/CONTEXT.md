...
2. [HYBRID] Compare retention across relevant cohorts and customer/value states while accounting for tenure, acquisition mix, seasonality, pricing/product changes, and censoring where they matter. Draw on `customer-optimization.retention.risk-segmentation` when its specialist method materially improves the analysis.
...
4. [AI] Prioritize interventions that improve underlying value realization and relationship quality over temporary lock-in or blanket discounting. Use `customer-optimization.retention.intervention-plan` when its specialist method is useful; neither method is a required execution step.
