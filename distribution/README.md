...
AURA distributions are portable-first and preserve the publisher/branding metadata, schemas, operating knowledge, navigation, and user-facing guidance needed by the included modules. They do not ship provider bindings, capability catalogs, environment capability inventories, or a runtime resolver. The active model/harness/user chooses real tools, providers, and Skills.
...
The packager resolves actual product-module dependencies, includes interface schemas needed to consume canonical objects referenced by included Workflows, removes excluded modules and maintainer-only or organization-specific state, preserves publisher provenance and AURA branding, creates edition-specific navigation and instance defaults, validates the resulting workspace, and emits a ZIP plus SHA-256 checksum.

## Tools and Skills

AURA does not maintain a parallel capability registry. Workflows describe the business outcome, evidence, constraints, and useful procedure in natural language. The active model/harness should use the best tools, providers, APIs, or specialist Skills actually available to it. A packaged edition does not choose or configure those execution resources, and missing host tooling does not activate an AURA fallback runtime.

## Portable-first rule

Every generated edition remains usable as a self-contained filesystem AURA distribution. By default, logical `instances/` and optional `runtime/runs/` state can live with the product folder. Recipients may instead configure a separate organization-owned workspace with `scripts/configure_workspace.py`; populated state moves through `scripts/migrate_workspace.py`. The human knowledge layer remains optional and ordinary Markdown. Git, Obsidian, hosted infrastructure, provider registries, custom schedulers, and the ViralTrac product are not required merely to use AURA.
...
If the user asks for work for which a useful AURA Playbook or Workflow is not installed, say so when that fact matters and continue with another valid method when the capable model/harness can do the work. Do not route the request to an imagined internal service or treat module presence as execution permission.
