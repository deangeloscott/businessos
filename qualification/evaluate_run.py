...
def main():
    ...
    for r,event in zip(results,queue['events']):
        ...
        review.append({'event_id':r['event_id'],'evaluation_id':event.get('evaluation_id'),'contract_id':event.get('contract_id'),'claim_under_test':test.get('claim_under_test'),'task':event['task'],'expected_sop_process_steps':test.get('process_steps',[]),'process_steps':test.get('process_steps',[]),'expected_completion_evidence':(test.get('claim_under_test') or {}).get('completion_evidence'),'completion_evidence':(test.get('claim_under_test') or {}).get('completion_evidence'),'competitive_profile':event.get('competitive_profile'),'hard_pass':r['hard_pass'],'hard_gates':r['hard_gates'],'evaluator_integrity_issues':r['evaluator_integrity_issues'],'integrity_flags':r['integrity_flags'],'run_integrity_flags':run_flags,'artifact_refs':receipt.get('artifact_refs',[]),'actual_artifacts':r.get('actual_artifacts',[]),'source_refs':receipt.get('source_refs',[]),'field_snapshot_refs':receipt.get('field_snapshot_refs',[]),'released_fixture_refs':receipt.get('released_fixture_refs',[]),'method_observations':receipt.get('method_observations',[]),'rubric_dimensions':dims,'score_scale':RUBRICS['score_scale'],'instructions':'Judge the actual business result first. For contract acceptance, use the SOP process as the expected business method/invariants, but do not require a particular Run ID, contract ledger, subcontract file, or evaluator-shaped receipt when the material work is demonstrably complete another legitimate way. Verify that requested work was genuinely performed, evidence is real, durable AURA state is truthful/useful, and the deliverable is professional for its audience.'})
