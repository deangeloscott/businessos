...
def product_snapshot(root):
    ...
    return {
        'format_version':'1.0','root':str(root),'files':files,
...
def snapshot_diff(before,after):
    ...
    b={x['path']:x.get('sha256') for x in before.get('files',[]) if visible(x['path'])}
    a={x['path']:x.get('sha256') for x in after.get('files',[]) if visible(x['path'])}
    return {
