...
def capture_checkpoint(product_root,workspace,run_dir,event_id,phase,business_id):
    product_root=Path(product_root).resolve(); workspace=Path(workspace).resolve(); run_dir=Path(run_dir).resolve()
    ...
    for name,cmd in {
        ...
        proc=subprocess.run(cmd,cwd=product_root,env=env,capture_output=True,text=True)
        validation[name]={'ok':proc.returncode==0,'returncode':proc.returncode,'stdout':proc.stdout[-4000:],'stderr':proc.stderr[-4000:]}
    snap={'event_id':event_id,'phase':phase,'business_id':business_id,'captured_at':now(),'workspace':tree_snapshot(workspace),'objects':collect_objects(workspace,business_id),'runs':collect_runs(workspace,business_id),'validation':validation}
    p=run_dir/'checkpoints'/event_id/f'{phase}.json'; write_json(p,snap)
    return p,snap


def main():
    ap=argparse.ArgumentParser(description='Maintainer-side before/after snapshot for an AURA qualification task.')
    ap.add_argument('event_id'); ap.add_argument('phase',choices=['before','after']); ap.add_argument('--business-id',required=True)
    ap.add_argument('--run-dir',required=True); ap.add_argument('--workspace',required=True); ap.add_argument('--product-root',required=True)
    a=ap.parse_args()
    p,_=capture_checkpoint(a.product_root,a.workspace,a.run_dir,a.event_id,a.phase,a.business_id)
    print(p)
