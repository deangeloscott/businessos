...
def classify(rd,event):
    eid=event['event_id'];before=rd/'checkpoints'/eid/'before.json';after=rd/'checkpoints'/eid/'after.json';receipt=_receipt_path(rd,event)
    state='terminal' if after.exists() and receipt.exists() else ('in_progress' if before.exists() or after.exists() or receipt.exists() else 'pending')
...
def _changed_runs(before,after):
    ...
    for row in (after or {}).get('runs',[]):
        rid=row.get('run_id')
        if not rid:continue
        nowv=(row.get('status'),row.get('method_type'),row.get('method_ref'),row.get('contract_id'))
        if b.get(rid)!=nowv:out.append(row)
    return out
...
def _run_refs(workspace,business_id,changed_runs):
    ...
    for row in changed_runs:
        ...
        for ref in (cont.get('result_refs') or [])+(cont.get('evidence_refs') or []):
...
def _existing_refs(refs,workspace):
    ...
    for ref in refs:
        ...
        if p.exists() and p.is_file() and p.stat().st_size>0:
            norm=str(p.resolve().relative_to(workspace.resolve())) if workspace.resolve() in p.resolve().parents else str(p.resolve())
            if norm not in out:out.append(norm)
    return out

def derive_receipt(rd,run,event,blocker_classification=None,blocker_detail=None):
    eid=event['event_id'];workspace=Path(run['workspace']);before=read_json(rd/'checkpoints'/eid/'before.json',{}) or {};after=read_json(rd/'checkpoints'/eid/'after.json',{}) or {};changed_objects=_changed_objects(before,after);changed_runs=_changed_runs(before,after);changed_paths=_changed_workspace_paths(before,after)
    run_refs,method_observations=_run_refs(workspace,event['business_id'],changed_runs);artifact_refs=[]
    for ref in run_refs+_asset_location_refs(workspace,changed_objects)+[p for p in changed_paths if _looks_like_artifact(p)]:
        if isinstance(ref,str) and ref not in artifact_refs:artifact_refs.append(ref)
    artifact_refs=_existing_refs(artifact_refs,workspace)
    ...
    material_result=bool(canonical_refs or artifact_refs)
    blocker=None
    if blocker_classification:
        ...
        status='blocked';blocker={'classification':'no_material_result','detail':'The controller observed no material persisted business result or deliverable for this task.'}
    receipt={'format_version':'3.0','generated_by':'qualification_controller','generated_at':now(),'event_id':eid,'business_id':event['business_id'],'status':status,'material_result_observed':material_result,'work_run_ids':[x.get('run_id') for x in method_observations if x.get('run_id')],'method_observations':method_observations,'artifact_refs':artifact_refs,'canonical_refs':canonical_refs,'source_refs':source_refs,'field_snapshot_refs':field_refs,'released_fixture_refs':released_refs,'summary':f'Controller observed {len(changed_objects)} changed canonical object(s), {len(artifact_refs)} usable artifact(s), and {len(method_observations)} optional work receipt(s).','blocker':blocker,'quality_notes':'Controller-generated observation only; professional quality and SOP effectiveness are evaluated from the actual work.'}
    write_json(_receipt_path(rd,event),receipt);return receipt
...
def main():
    ...
    p=sub.add_parser('finish');p.add_argument('run_dir');p.add_argument('--event-id');p.add_argument('--blocker-classification',choices=sorted(VALID_BLOCKERS));p.add_argument('--blocker-detail');a=ap.parse_args()
    result=start(a.run_dir,a.event_id) if a.command=='start' else (finish(a.run_dir,a.event_id,a.blocker_classification,a.blocker_detail) if a.command=='finish' else status(a.run_dir));print(json.dumps(result,indent=2))
