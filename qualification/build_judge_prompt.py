...
3. For a workflow-acceptance event, treat the SOP process steps as the expected **business method and quality invariants**, not as a demand for exact internal files. Equivalent or better implementation may pass; skipping a material research/analysis/production/QA step should reduce `method_rigor`, completeness, and relevant outcome dimensions.
4. Do **not** require a particular Run ID, workflow ID in a receipt, subcontract ledger, checkpoint-shaped artifact, provider, model, or tool-routing trace as proof of quality. Optional method provenance can help reconstruct what happened but is not the business result.
