...
def main():
    ...
    for m in suite.get('concurrency_missions',[]):
        e={'event_id':m['id'],'kind':'concurrency_mission','business_id':'qa-atlasops-saas','fixture':m['fixture'],'contract_id':None,'task':m['task'],'competitive_profile':'concurrency','required_output':{'actual_output_not_description':True},'rubric_dimensions':[x['id'] for x in RUBRICS['base']]+RUBRICS['profiles']['concurrency_system'],'receipt_path':f"candidate-results/{m['id']}.json",'lane':m['lane']}; events.append(e)
    for sub in ('candidate-a','candidate-b','candidate-results','checkpoints','evaluator'): (run_dir/sub).mkdir(parents=True,exist_ok=True)
    master={'format_version':'1.0','run_id':run_id,'profile':'concurrency','events':events,'event_count':len(events)}
    write_json(run_dir/'master-queue.json',master)
