...
This regression intentionally focuses on the invariants unique to the AURA boundary:
- explicit organization truth is grounded rather than model-invented;
- outward business claims remain evidence-bounded;
- valid work does not require a Run or AURA playbook;
- the front door retrieves/recommends without taking over execution;
...
def main():
    ...
    try:
        ...
        for obj in objects:
            path=_path(BASE,obj);write_json(path,obj)

        errors,_,counts=validate_business(BID,True)
        require(not errors,f'grounded organization context should validate: {errors}')
        require(counts.get('Business')==1 and counts.get('ProductService')==3,f'expected grounded business/service context: {counts}')

        # Model plausibility is not permission to enlarge explicit organization truth.
        market=next((BASE/'context/markets').glob('*.json'));original=market.read_text();obj=json.loads(original)
        obj['name']='Maryland';obj['geography']='Maryland';market.write_text(json.dumps(obj,indent=2)+'\n')
        errors,_,_=validate_business(BID,True)
        require(any('not grounded' in e and 'Maryland' in e for e in errors),f'unsupported explicit-user expansion should fail: {errors}')
        market.write_text(original)

        # Persist one explicit reusable business claim grounded to the original user source.
        ...
        # A truthful customer-facing Asset is valid without a Run, contract, ActionPacket,
        # or approval object. AURA validates organization truth, not an execution ceremony.
        html=BASE/'assets/claim-regression.html';html.parent.mkdir(parents=True,exist_ok=True)
        ...
        asset={
            ...
            'extensions':{'businessos':{
                ...
                'claim_manifest':[{'text':'We provide written estimates.','classification':'approved_business_claim','support_refs':[claim['id']]}]
            }}
        }
        ...
        # The same artifact becomes invalid when it enlarges the supported promise.
        html.write_text('<html><body><p>We guarantee same-day written estimates.</p></body></html>')
        asset['extensions']['businessos']['claim_manifest']=[{
            'text':'We guarantee same-day written estimates.','classification':'approved_business_claim','support_refs':[claim['id']]
        }]
        write_json(ap,asset)
        errors,_,_=validate_business(BID,True)
        require(any('enlarges supported promise' in e for e in errors),f'unsupported outward promise expansion should fail: {errors}')
        html.write_text('<html><body><p>We provide written estimates.</p></body></html>')
        asset['extensions']['businessos']['claim_manifest']=[{'text':'We provide written estimates.','classification':'approved_business_claim','support_refs':[claim['id']]}]
        write_json(ap,asset)

        # Selecting an AURA playbook enriches context; it does not create a Run or take
        # authority away from the active model/user/harness.
        ...
        require('not authority' in prepared.get('recommended_playbook',{}).get('rule',''),f'playbook recommendation became execution authority: {prepared}')
        ...
        # Physical absence matters: do not keep retired architecture around as dormant baggage.
        retired=[
            ROOT/'core/policies/approval.md',
            ROOT/'core/policies/operating-scope.md',
            ROOT/'core/policies/customer-facing-mutations.md',
            ROOT/'core/schemas/action/action-packet.schema.json',
            ROOT/'core/schemas/action/approval.schema.json',
            ROOT/'core/contracts/action-control',
            ROOT/'scripts/validate_customer_facing_mutations.py',
            ROOT/'scripts/capture_customer_facing_state.py',
            ROOT/'scripts/build_mutation_claim_manifest.py',
        ]
        for path in retired:
            require(not path.exists(),f'retired control-plane artifact reappeared: {path.relative_to(ROOT)}')

        errors,_,_=validate_business(BID,True)
        require(not errors,f'current architecture should finish with valid organization state: {errors}')
        print('agent hardening regressions passed: truth and continuity remain strong without AURA execution-control baggage')
    finally:
        if BASE.exists():shutil.rmtree(BASE)
