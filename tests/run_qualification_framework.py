...
Protect evaluator isolation, truthful ordinary requests, Workflow coverage, artifacts/evidence
quality gates, and optional continuity. Do not make internal capability vocabularies,
execution ledgers, or exact orchestration shapes part of product correctness.
"""
...
from prepare_run import init_business,apply_candidate_request,candidate_surface,_ensure_separate,select_events
...
def smoke_prepare():
    ...
    with tempfile.TemporaryDirectory(prefix='aura-qualification-smoke-') as td,tempfile.TemporaryDirectory(prefix='aura-workspaces-smoke-') as cd:
        ...
        for wid in selected:cmd+=['--contract',wid]
        ...
        events=queue.get('events',[]);req(len(events)==len(selected) and {x.get('contract_id') for x in events}==set(selected),'representative Workflow targets were not preserved evaluator-side');req(all(str(x.get('event_id','')).startswith('TASK-') for x in events),'candidate task IDs should be opaque')
...
def main():
    ...
    target='seo.intelligence.organic-competition.page-analysis';ordinary='Analyze a strong current organic page for a valuable AtlasOps search intent and identify the material gaps for our business.';customized=apply_candidate_request([{'contract_id':target,'task':'generic'}],ordinary);req(customized[0]['task']==ordinary,'ordinary candidate request was not preserved')
    try:apply_candidate_request([{'contract_id':target,'task':'generic'}],f'Execute {target}')
    ...
    suite=build();manifest=json.loads((ROOT/'SYSTEM-MANIFEST.json').read_text());expected=manifest.get('workflow_count') or manifest.get('contract_count');req(suite['workflow_count']==expected,f"qualification Workflow coverage {suite['workflow_count']} != manifest {expected}");tests=suite['workflow_tests'];ids=[t['workflow_id'] for t in tests];req(len(ids)==len(set(ids)),'duplicate Workflow qualification tests');req(all(not t['unknown_composed_workflows'] for t in tests),'unknown composed Workflows in qualification suite');req(all(t['hard_gates'] and t['rubric_dimensions'] and t['candidate_task'] for t in tests),'every Workflow needs gates, rubric, and ordinary task');req(all('capabilities' not in t for t in tests),'retired capability ontology leaked into qualification cases')
    ...
    ecosystem=next(t for t in tests if t['workflow_id']=='core.intelligence.ecosystem-radar');req(all(isinstance(x,str) for x in ecosystem['normally_used_workflows']),'Workflow composition metadata should normalize to IDs');customer=[t for t in tests if t.get('artifact_role')=='customer_facing_production_root'];req(customer and all(t['output_policy']['artifact_required'] and 'actual_artifact_exists' in t['hard_gates'] for t in customer),'customer-facing roots must prove actual artifacts')
    ...
    req([t for t in tests if t['competitive_profile']=='search_live_field'],'SEO/AEO live-field tests missing');req([t for t in tests if t['competitive_profile']=='paid_and_persuasion_field'],'competitive marketing tests missing');seed_source=inspect.getsource(init_business);req('bootstrap_explicit_context.py' in seed_source and '--require-context' in seed_source,'qualification setup must ground fixture context canonically');req('hidden-fixtures' in seed_source and 'timeline' in seed_source,'longitudinal evidence path missing')
    ...
    print(f"qualification framework regressions passed: {suite['workflow_count']} Workflow tests, durable principles/ledger, real artifact/evidence gates, blind candidate isolation, longitudinal releases, and optional Workflow receipt smoke passed")
