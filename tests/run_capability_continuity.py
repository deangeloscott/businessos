...
def main():
    required=[
        ...
        'scripts/manage_local_capabilities.py','scripts/register_scheduler_binding.py','scripts/list_due_monitoring.py'
    ]
    for rel in required:
    ...
    for phrase in ['semantic cadence is not the same thing as an active background schedule','User-specified cadence wins','Due-on-next-start','Missing background automation changes the executor, not the monitoring plan']:
    ...
    if 'monitoring_cadence' not in src_schema.get('properties',{}):fail('SourceProfile missing semantic monitoring cadence')
    if 'monitoring_cadence' in src_schema.get('required',[]):fail('monitoring cadence must remain backward-compatible/optional')
    ...
    try:
        ...
        # A later inferred default may not silently override the user's cadence.
        blocked=run(upsert+[
        ...
        subject=next(x for x in due['subjects'] if x.get('subject_key')=='example_creator')
        if subject.get('execution_status')!='planned_unbound' or not subject.get('needs_on_start_attention'):fail(f'unbound cadence was represented as scheduled: {subject}')
        ...
        subject=next(x for x in active['subjects'] if x.get('subject_key')=='example_creator')
        if subject.get('execution_status')!='active_automatic' or subject.get('needs_on_start_attention'):fail(f'verified scheduler binding did not resolve automatic execution truth: {subject}')
        ...
        for phrase in ['Tracked sources/surfaces:** `2`','Automatic execution:** `active automatic`','weekly','monthly','sched_example_creator']:
        ...
        print('AURA capability + continuity regressions passed: scheduler truth/fallback, per-subject cadence protection, grouped human knowledge, trusted local packs, safe system-change authorization, and binding-preserving host refresh')
    finally:
