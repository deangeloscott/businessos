...
def main():
    ...
    try:
        ...
        for eid in targets:
            ...
            req((pdir/'generated/contract-registry.json').exists(),f'{eid} registry was not generated');req((pdir/'generated/workflow-candidate-index.json').exists(),f'{eid} Workflow discovery index missing');req((pdir/'PLAYBOOKS.md').exists(),f'{eid} high-level Playbook catalog missing');req((pdir/'WORKFLOW-INDEX.md').exists(),f'{eid} Workflow index missing');req((pdir/'skills/viraltrac-aura/SKILL.md').exists(),f'{eid} AURA awareness Skill missing');req(not (pdir/'core/capabilities/catalog.json').exists(),f'{eid} packaged retired capability ontology')
