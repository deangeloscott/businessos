...
def main():
    ...
    for retired in ('artifact_role','competitive_profile','output_policy','normally_used_workflows'):
        req(all(retired not in t for t in tests),f'qualification cases retained inferred semantic field {retired}')
    ...
    req(all('authored_workflow_refs' in t and 'unknown_workflow_refs' in t for t in tests),'qualification lost reference-integrity visibility for authored supporting knowledge')
    req(all(not t['unknown_workflow_refs'] for t in tests),'qualification contains unresolved authored Workflow references')
    ...
    for retired_phrase in ('output_policy','competitive_profile','artifact_role','competitive_field_evidence_reconstructable','actual_artifact_exists'):
        req(retired_phrase not in evaluator,f'evaluator retained inferred semantic hard gate: {retired_phrase}')
    ...
    controller=(ROOT/'qualification/task_controller.py').read_text();req('matching root AURA Run' not in controller and "'material_result_observed':material_result" in controller,'controller must classify observed business results rather than execution-ledger completion')
