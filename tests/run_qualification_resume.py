...
def main():
    with tempfile.TemporaryDirectory(prefix='aura-qualification-resume-') as td:
        ...
        events=[
            {'event_id':'TASK-0001','evaluation_id':'E1','kind':'workflow_acceptance','business_id':'atlasops','workflow_id':'content.intake.content-brief','task':'Create a useful content brief for the active business.','receipt_path':'evaluator/receipts/TASK-0001.json'},
            {'event_id':'TASK-0002','evaluation_id':'E2','kind':'workflow_acceptance','business_id':'atlasops','workflow_id':'content.production.article','task':'Create the requested customer-facing article for the active business.','receipt_path':'evaluator/receipts/TASK-0002.json'}
        ]
        ...
        run_dir=ws/'runtime/runs/atlasops/run_interrupted'; run_dir.mkdir(parents=True); (run_dir/'run.json').write_text(json.dumps({'run_id':'run_interrupted','business_id':'atlasops','method_type':'aura_workflow','method_ref':'content.production.article','workflow_id':'content.production.article','status':'active'})+'\n')
        ...
        first=s['first_unfinished']; req(first['event_id']=='TASK-0002' and first['state']=='in_progress','must resume at interrupted second task'); req(first.get('workflow_id')=='content.production.article','recovery lost Workflow identity'); req(any(r['run_id']=='run_interrupted' for r in first['event_runs']),'active optional task Run must be surfaced maintainer-side for recovery')
    ...
    print('qualification interruption recovery regressions passed with Workflow-native evaluator-side recovery')
