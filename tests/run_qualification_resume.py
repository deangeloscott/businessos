...
def main():
    with tempfile.TemporaryDirectory(prefix='aura-qualification-resume-') as td:
        ...
        events=[
            ...
            {'event_id':'TASK-0002','evaluation_id':'E2','kind':'contract_acceptance','business_id':'atlasops','contract_id':'content.intake.work-request','task':'Turn the current content need into actionable work for the active business.','receipt_path':'evaluator/receipts/TASK-0002.json'}
        ]
        ...
        run_dir=ws/'runtime/runs/atlasops/run_interrupted'; run_dir.mkdir(parents=True); (run_dir/'run.json').write_text(json.dumps({'run_id':'run_interrupted','business_id':'atlasops','contract_id':'content.intake.work-request','status':'active'})+'\n')
        ...
        recovery=rr/'evaluator/RECOVERY.md'; req(recovery.exists(),'maintainer recovery instructions were not written'); text=recovery.read_text().lower(); req('candidate/model should still receive only the normal aura product/workspace and an ordinary business request' in text,'recovery must preserve blind candidate boundary'); req('turn the current content need into actionable work' in text,'recovery must preserve original ordinary business request'); req(not (rr/'candidate').exists(),'recovery created candidate-visible qualification files')
