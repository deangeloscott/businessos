...
def main():
    ...
    events=queue.get('events',[]);req(run.get('mode')=='workflow-diagnostic' and len(events)==1,'focused Workflow diagnostic did not remain one explicit diagnostic');event=events[0];req(event.get('kind')=='workflow_diagnostic' and event.get('workflow_id')==workflow_id,'focused Workflow target was not retained evaluator-side');req(event.get('task')==ordinary,'focused diagnostic did not preserve maintainer-authored ordinary request');req(event.get('claim_under_test') and event.get('workflow_process_steps'),'focused diagnostic lost authored Workflow knowledge for professional review');req(event.get('rubric_dimensions'),'focused diagnostic lost professional review dimensions')
