...
def run_prepare(args,prefix,temp_root):
    temp_root=Path(temp_root)
    td=temp_root/f'{prefix}-evaluator';cd=temp_root/f'{prefix}-candidate'
    cmd=[sys.executable,str(ROOT/'qualification/prepare_run.py'),*args,'--run-root',str(td),'--candidate-root',str(cd),'--run-id','smoke']
    ...
    rd=td/'smoke';run=json.loads((rd/'run.json').read_text());queue=json.loads((rd/'evaluator/queue.json').read_text());product=Path(run['product_root']);workspace=Path(run['workspace']);surface=Path(run['candidate_surface_root'])
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='aura-usecase-library-') as temp_root:
        rd,run,queue=run_prepare(['--case','saas-positioning-page'],'usecase-smoke',temp_root)
        ...
        rd,run,queue=run_prepare(['--case','saas-memory-evolution'],'longitudinal-smoke',temp_root)
        ...
        rd,run,queue=run_prepare(['--workflow',workflow_id,'--fixture','atlasops-saas','--request',ordinary],'workflow-diagnostic-smoke',temp_root)
