...
5. **QA must show checks, not merely a verdict.** QA completion requires a matching structured pass record with substantive criterion, method, finding/evidence, issue/correction/limitation state, and per-check outcomes. Content QA over a text target must bind each applicable pass to a literal target excerpt; absent features are `not_applicable` with a reason, not fictional passes. Asset QA must name an existing non-self target Asset and its exact version; a QA wrapper created by the same QA Run cannot inspect itself. `pass` requires no material failed checks or unresolved blockers. A bare JSON object whose only meaningful assertion is `status: pass`, or a list of generic strings asserting that checks occurred, is not QA evidence.
6. **Intelligence must preserve the analysis behind the conclusion.** Content intelligence that writes a concise Observation, Insight, or Learning must also leave a Run-local work record with scope, method, item-level evidence sample, actual comparisons/baselines, findings/mechanisms, alternatives, limitations, and recommended action. Each sampled item needs a literal support excerpt present in captured evidence, and every sample/finding reference must resolve. The canonical conclusion is durable organizational state; it is not a substitute for the evidence and analysis used to reach it.
...
- presentation: rendered slides, or a complete multi-slide specification with visible content, visual direction, speaker notes, proof/source attribution, and audience decision flow when the contract permits it;
...
- podcast: recorded audio, or a complete episode script/host packet with ordered segments, transitions, evidence notes, CTA, audio/edit direction, and show notes when the contract permits it.
