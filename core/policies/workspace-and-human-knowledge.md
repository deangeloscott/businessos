...
ViralTrac AURA separates **product source**, **organization-owned memory/state**, **human knowledge views**, and **external authoritative data**. This separation improves portability, upgrades, collaboration, and human usability without changing the meaning of canonical AURA objects.
...
- Durable organization-owned state belongs to the active AURA workspace.
...
- If the current workspace contains organization state that is absent from a new target, do not silently switch roots. Use `scripts/migrate_workspace.py` or an explicitly authorized intentional state switch.

## 2. Active workspace namespaces
The active workspace owns these logical namespaces:
- `instances/<business-id>/` — canonical durable organization memory/state.
- `runtime/runs/<business-id>/<run-id>/` — optional bounded work receipts/continuity when useful; not AURA execution authority.
...
A workspace move is organization-state migration, not a product fork.
...
- Canonical JSON objects remain authoritative for AURA state.
...
- A generated Markdown summary, Obsidian page, dashboard, or future interface must not silently become an independent competing truth store.
- Generated pages should preserve canonical IDs/source refs so humans and agents can trace the underlying state.

## 5. Obsidian and other human interfaces
...
- Do not require Git for the simple one-folder setup.
...
- Optional work receipts, logs, caches, and other ephemeral files need not be versioned merely because the workspace itself is versioned.

## 7. Workspace profiles
AURA exposes three human-facing workspace/storage choices in `distribution/deployment-profiles.json`:
1. `simple` — download/unzip and use locally.
2. `power_user` — separate organization workspace, optional private Git/versioning, optional human knowledge interface.
3. `organization` — separate private team workspace, controlled collaboration/versioning, optional human knowledge interface.

These describe where organization-owned AURA memory lives and how people may collaborate with it. They are not execution modes, runtime tiers, or separate products with different operating semantics.
...
The public product name is ViralTrac AURA; `BusinessOS` remains only where a compatibility namespace/name still has a concrete reason to exist. See `BRANDING.md`.
