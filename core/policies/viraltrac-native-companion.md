...
The host/harness owns authenticated retrieval. Do not put credentials in this workspace. When filesystem access is available, pass the non-secret descriptor/capability response through `scripts/sync_viraltrac_capabilities.py` so AURA stores only a minimized capability snapshot and validated provider-neutral bindings.
...
A model/harness may choose the exact current ViralTrac transport/tool from discovered descriptors. AURA should express the business capability/result it needs rather than hardcoding ViralTrac's internal tables or duplicating route logic.
...
- AURA/BusinessOS remains authoritative for its operating process, domain ownership, Insights, Opportunities, Initiatives, ActionPackets, OutcomeEvaluations, and Learning.
...
- ViralTrac recommendations/evidence are inputs to AURA reasoning; they do not silently become canonical Insights/Opportunities without the owning process.

Use `core/providers/viraltrac/object-mapping.json` for the intended interop boundary.

## 4. Do not duplicate the data plane
Do not copy ViralTrac's raw/large operational history into `instances/<business-id>/` merely because it is accessible. Preserve durable AURA intelligence plus references to authoritative ViralTrac results/artifacts/receipts.
...
AURA first decides and authorizes the intended business Action. ViralTrac may then be used as a governed handoff/coordination layer when the specific action is supported.

Preferred sequence where advertised by the connected capability surface:

`AURA ActionPacket -> ViralTrac propose -> preview -> required confirmation/approval -> execute or external-execution packet -> receipt -> AURA ChangeEvent -> VerificationRecord`

The generic `business.action.governed.*` capabilities never prove that a specific destination mutation is supported. The target-specific capability, current execution mode, connection health, consent/suppression/eligibility policy, AURA authorization, and any user approval still apply.

## 7. Measurement and learning loop
For an executed or externally completed initiative, prefer available ViralTrac measurement/outcome evidence when it is an authoritative/relevant source. Link its result/receipt through `SourceRecord` and canonical MetricObservation objects, then let `core.measurement.evaluate-outcome` produce the AURA/BusinessOS OutcomeEvaluation.
...
When ViralTrac is connected, treat its Event / Reactive Plane governed event/reaction plane as a supported AURA acceleration path, not merely a future concept. AURA is still an external operating consumer: ViralTrac owns occurrence truth, normalization, identity/policy enrichment, delivery/evaluation receipts, native/provider effects, trace/replay governance, and runtime readiness; AURA owns business materiality, installed-domain routing, its operating objects, and its own authorization/learning loop.
