...
## Purpose
Bring a human-authored Markdown/text note into AURA as provenance-backed source material without assuming the note is verified canonical business truth.

## Business Outcome
Let humans contribute knowledge naturally through a Markdown-style workspace while preserving AURA evidence, truth, and interpretation boundaries.

## Run When
Use when a human asks AURA to use, review, incorporate, or learn from a note stored under `knowledge/<business-id>/notes/`.
...
4. [HYBRID] For directly supported decision-relevant statements that should become reusable evidence, use `core.intelligence.publish-observation` or another appropriate evidence/context method when it helps. This is optional operating knowledge, not a required handoff stage.
5. [AI] Preserve interpretations, questions, or candidate insights through the organizational memory type that naturally owns their meaning when doing so will materially help future work. Use relevant domain operating knowledge directly when useful; do not create an internal routing handoff merely because another area is relevant.
...
## Completion Criteria
- The note is traceable as noncanonical source material and any useful durable meaning is preserved through the appropriate AURA memory/evidence path without internal routing ceremony.
