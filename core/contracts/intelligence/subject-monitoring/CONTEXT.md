...
12. [DETERMINISTIC] Refresh the human knowledge layer when useful. Normal-user completion should point to the human concept first (for example `AtlasOps → Knowledge → Tracked Subjects → <subject>`) and include raw canonical/runtime paths only when they help an advanced operator/debugger. The user/model/harness can inspect current monitoring with `scripts/monitoring_status.py <business-id>` and adjust cadence/notification intent through normal AURA requests without knowing internal paths.
