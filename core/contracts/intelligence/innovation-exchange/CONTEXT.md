...
## Purpose
Prepare, import, discover, and evaluate portable BusinessOS process improvements without making a hosted exchange, telemetry, or automatic sharing part of the required runtime.

## Business Outcome
Let users learn from useful BusinessOS innovations created elsewhere while retaining local ownership, privacy, portability, and evidence standards.
...
4. [HUMAN] Export/share only through `scripts/export_innovation_package.py ... --approve` or an equivalent explicit current-task approval. No background upload or automatic submission is allowed.
5. [DETERMINISTIC] Validate imported JSON/ZIP packages with `scripts/validate_innovation_package.py`, then use `scripts/import_innovation_package.py` to store the contribution, compatibility state, SourceRecord, candidate Insight, and InnovationExchangeEntry.
...
7. [DETERMINISTIC] Use `scripts/build_innovation_exchange_index.py` to publish an approved folder/repository manifest and `scripts/browse_innovation_exchange_index.py` to search a downloaded index before importing. Use `scripts/list_innovation_exchange.py` for the active business's already-imported local feed. Any hosted registry/API is only another discovery transport.
8. [HYBRID] When the active business tests an imported innovation, preserve the OutcomeEvaluation and record the local result through `scripts/record_innovation_outcome.py`.
...
## Verification
- No external submission occurred without explicit approval.
...
- The exchange can be unused/disconnected without breaking BusinessOS.

## Completion Criteria
- Sharing/import/discovery/evidence state is inspectable and portable, and any adopted process still passes normal BusinessOS learning/authorization rules.
