...
AURA SOP contract `reads` / `writes` metadata should refer only to these canonical organization-owned object types.
...
Before adding a new canonical object type, demonstrate that organizational memory, operational knowledge, continuity, truth, measurement, Learning, or AURA integrity materially worsens without a distinct durable representation.

If the information is:

- execution/runtime mechanics → leave it to the harness/runtime;
- process-specific → keep it in the SOP or its evidence/output;
